...
numeric_cols = ["Age","Poss","MP","Starts","Min","90s","Gls","Ast","G+A","G-PK","PK","PKatt","CrdY","CrdR","xG","npxG","xAG","npxG+xAG","PrgC","PrgP"]
df[numeric_cols] = df[numeric_cols].apply(pd.to_numeric, errors="coerce")
df = df[df["Min"] > 0].dropna(subset=["Squad"])

# Derived per-player stats
for col, new in [("Gls", "Goals_per_90"), ("Ast", "Assists_per_90"), ("PrgC", "PrgC_per_90"), ("PrgP", "PrgP_per_90")]:
    df[new] = df[col] / (df["Min"] / 90)
df = df.assign(xG_diff=df["Gls"]-df["xG"], xGA_diff=df["G+A"]-(df["xG"]+df["xAG"]), npxGxAG_per_90=df["npxG+xAG"]/(df["Min"]/90), Discipline=(df["CrdY"]+2*df["CrdR"])/df["Min"])

# Aggregate per squad
agg = df.groupby("Squad").agg({"Goals_per_90":"mean","Assists_per_90":"mean","xG_diff":"mean","xGA_diff":"mean","npxGxAG_per_90":"mean","Poss":"mean","PrgC_per_90":"mean","PrgP_per_90":"mean","Discipline":"mean"}).reset_index()
