...
class MinecraftItemLookupApp:
    def __init__(self, root):
        ...
        self.search_label = ttk.Label(
            self.search_frame, text="Search for Minecraft item:"
        )
        ...
        self.search_button = ttk.Button(
            self.search_frame, text="Search", command=self.search_item
        )
        ...
        self.item_info = scrolledtext.ScrolledText(
            self.results_frame, wrap=tk.WORD, font=("Arial", 10), padx=10, pady=10
        )
    ...
    def show_suggestions(self, query):
        ...
        suggestions_from_difflib = difflib.get_close_matches(
            normalized_user_input, all_item_names, n=10, cutoff=0.6
        )
        ...
        if combined_suggestions:
            ...
            self.show_status(
                f"No exact match for '{query}'. Select a suggestion or try another search."
            )
            self.show_suggestions_widgets()
        else:
            self.hide_suggestions()
            self.show_status(
                f"Sorry, no matches found for '{query}'. Try another item."
            )
            self.item_info.config(state=tk.NORMAL)
            self.item_info.delete(1.0, tk.END)
            self.item_info.insert(
                tk.END,
