...
def get_minecraft_item_info(item_name: str) -> str:
    ...
    if normalized_item_name in item_database:
        ...
        return "" # Return empty string if not found, to handle suggestions in main loop
    ...
    while True:
        ...
        if info: # If exact match found
            ...
            if display_suggestions:
                print(f"Did you mean one of these? Please select a number or re-enter your query:")
                for i, item in enumerate(display_suggestions):
                    print(f"{i + 1}. {item.title()}")

                try:
                    choice = input("Enter your choice (number) or press Enter to re-enter query: ").strip()
                    if choice.isdigit():
                ...
                print(f"Sorry, I don't have information for '{user_input}' and couldn't find any close matches. Please try another item.")
