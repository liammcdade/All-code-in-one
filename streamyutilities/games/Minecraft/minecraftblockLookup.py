# minecraft_item_lookup.py
import difflib

# A dictionary to store Minecraft item data
# Key: Item name (lowercase)
# Value: A dictionary containing 'uses' and 'how_to_get'
item_database = {
    # === Basic/Core Items ===
    "wood planks": {
    ...
    "porkchop": {"uses": "Restores hunger.", "how_to_get": "Dropped by pigs."},
    "cooked porkchop": {
...
def get_minecraft_item_info(item_name: str) -> str:
    """
    Provides information about a specific Minecraft item, including its uses
    and how to obtain it.

    Args:
        item_name (str): The name of the Minecraft item to look up (case-insensitive).

    Returns:
        str: A string containing the item's description, or an empty string if not found.
    """
    # Normalize the item name for lookup
    normalized_item_name = item_name.lower().strip()

    if normalized_item_name in item_database:
        info = item_database[normalized_item_name]
        return (
            f"**{item_name.title()}**:\n"
            f"- **Uses**: {info['uses']}\n"
            f"- **How to get**: {info['how_to_get']}"
        )
    else:
        return (
            ""  # Return empty string if not found, to handle suggestions in main loop
        )


# --- Example Usage ---
if __name__ == "__main__":
    print("Welcome to the Minecraft Item Lookup!")
    print("Type 'exit' to quit.")

    while True:
        user_input = input("\nEnter the name of a Minecraft item: ").strip()

        if user_input.lower() == "exit":
            print("Exiting Minecraft Item Lookup. Goodbye!")
            break

        info = get_minecraft_item_info(user_input)

        if info:  # If exact match found
            print(info)
        else:  # No exact match, try to suggest
            # Get all available item names for suggestions
            all_item_names = list(item_database.keys())
            normalized_user_input = user_input.lower()

            # Step 1: Get close matches with a lower cutoff for broader results
            suggestions_from_difflib = difflib.get_close_matches(
                normalized_user_input, all_item_names, n=10, cutoff=0.4
            )

            # Step 2: Add substring matches as a fallback/enhancement
            substring_suggestions = set()
            if (
                len(normalized_user_input) > 1
            ):  # Only do substring for inputs longer than 1 character
            ...
            combined_suggestions = list(
                dict.fromkeys(
                    suggestions_from_difflib + sorted(list(substring_suggestions))
                )
            )

            # Limit the number of suggestions to display for readability
            display_suggestions = combined_suggestions[:10]  # Show up to 10 suggestions

            if display_suggestions:
                print(
                    f"Did you mean one of these? Please select a number or re-enter your query:"
                )
                for i, item in enumerate(display_suggestions):
                    print(f"{i + 1}. {item.title()}")

                try:
                    choice = input(
                        "Enter your choice (number) or press Enter to re-enter query: "
                    ).strip()
                    if choice.isdigit():
                        choice_index = int(choice) - 1
                        if 0 <= choice_index < len(display_suggestions):
                            selected_item = display_suggestions[choice_index]
                            print(get_minecraft_item_info(selected_item))
                        else:
                ...
                print(
                    f"Sorry, I don't have information for '{user_input}' and couldn't find any close matches. Please try another item."
                )
