...
def resize_image(
    image_path, output_path, new_width=None, new_height=None, scale_percent=None
):
    ...
    try:
        ...
        if scale_percent is not None:
            if not (0 < scale_percent <= 500):
                print(
                    f"Warning: Scale percent {scale_percent}% is out of reasonable range (1-500). Skipping {image_path}"
                )
                return False
            ...
            print(
                f"Error: Invalid resize parameters for {image_path}. Width/Height or Scale must be provided."
            )
            return False

        if new_width <= 0 or new_height <= 0:
            print(
                f"Warning: Calculated dimensions ({new_width}x{new_height}) are invalid for {image_path}. Skipping."
            )
            return False

        print(
            f"Processing {image_path}: original ({original_width}x{original_height}) -> new ({new_width}x{new_height})"
        )
        ...
        print(
            f"Skipping {image_path}: Cannot identify image file or format is not supported."
        )
        return False
    except FileNotFoundError:
        print(
            f"Skipping {image_path}: Source file not found (should not happen if script logic is correct)."
        )
        return False
...
def main():
    ...
    parser.add_argument(
        "source_dir", help="Path to the directory containing original images."
    )
    parser.add_argument(
        "output_dir", help="Path to the directory where resized images will be saved."
    )
    ...
    group.add_argument(
        "--scale", type=float, help="Percentage to scale images by (e.g., 50 for 50%)."
    )
