    ...
    try:
        ...
        if file_content_lines is None:  # If all encodings failed
            if not files_with_matches:  # Only print error if not just listing files
                print(
                    f"Warning: Could not decode file {filepath} with tried encodings. Skipping."
                )
            return False
        ...
        return (
            file_matched  # True if any line matched (for non -l mode), False otherwise
        )
...
def main():
    ...
    parser.add_argument(
        "pattern", help="The text string or regular expression to search for."
    )
    parser.add_argument(
        "paths",
        nargs="+",
    ...
    for path_arg in args.paths:
        if os.path.isfile(path_arg):
            ...
            print(
                f"Warning: Path '{path_arg}' is not a valid file or directory. Skipping."
            )
    ...
    overall_match_found = (
        False  # To know if anything was printed, for exit status ideas
    )
