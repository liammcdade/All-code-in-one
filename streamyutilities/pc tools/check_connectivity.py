    ...
    try:
        ...
        if stdout:
            ...
            if platform.system().lower() == 'windows':
                ...
                loss_match = re.search(r"(\d+) packets transmitted, (\d+) received, (?:[\d\.]+% packet loss|(\d+)% packet loss)", stdout)
                if loss_match:
            ...
            if rtt_avg is not None:
                summary_lines.insert(2, f"  RTT (ms): Min={rtt_min}, Avg={rtt_avg}, Max={rtt_max}" +
        ...
        if stderr:
            ...
            if "not found" in stderr.lower() or "not recognized" in stderr.lower():
                summary_lines.append("  Error: 'ping' command not found. Ensure it is installed and in your PATH.")
