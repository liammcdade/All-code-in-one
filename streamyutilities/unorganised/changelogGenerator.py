...
def run_git_command(command_list):
    ...
    try:
        process = subprocess.run(
            command_list, capture_output=True, text=True, check=False, encoding="utf-8"
        )
...
def get_commit_details(commit_hash, format_str="%s"):
    ...
    stdout, _, retcode = run_git_command(
...
def generate_changelog_data(from_revision, to_revision, excluded_types_list):
    ...
    if retcode != 0:
        ...
        for rev in [from_revision, to_revision]:
            _, _, rev_retcode = run_git_command(
                ["git", "rev-parse", "--verify", f"{rev}^{{commit}}"]
            )
            if rev_retcode != 0:
                print(
                    f"Error: Revision '{rev}' is not a valid Git revision.",
                    file=sys.stderr,
                )
        return None, None
...
def print_markdown_changelog(categorized_commits, breaking_changes, from_rev, to_rev):
    ...
    if (
        DEFAULT_MISC_SECTION_TITLE in categorized_commits
        and DEFAULT_MISC_SECTION_TITLE not in sorted_sections
    ):
    ...
    for section_title in sorted_sections:
        if section_title in categorized_commits:
            ...
            for commit in commits_in_section:
                ...
                if (
                    section_title == DEFAULT_MISC_SECTION_TITLE and not commit["type"]
                ):  # Was not parsed as conventional
...
if __name__ == "__main__":
    ...
    _, _, retcode_git_repo = run_git_command(
    ...
    if not from_revision:
        ...
        if not from_revision:
            print(
                "Error: Could not determine default '--from-rev' (no tags found). Please specify a starting revision.",
                file=sys.stderr,
            )
            return
    ...
    excluded_commit_types = set(
        t.strip().lower() for t in args.exclude.split(",") if t.strip()
    )

    print(f"Generating changelog from {from_revision} to {to_revision}...")
    if excluded_commit_types:
        print(
            f"Excluding commit types: {', '.join(sorted(list(excluded_commit_types)))}"
        )

    # --- Generate Data ---
    categorized_data, breaking_changes_data = generate_changelog_data(
        from_revision, to_revision, excluded_commit_types
    )
    ...
    if not categorized_data and not breaking_changes_data:
        print(
            "No changes found between the specified revisions that match the criteria."
        )
        return

    # --- Output Changelog ---
    if args.format == "markdown":
        print_markdown_changelog(
            categorized_data, breaking_changes_data, from_revision, to_revision
        )
    else:
