...
try:
    ...
    print(
        "Error: The 'qrcode' library is required. Please install it (e.g., 'pip install \"qrcode[pil]\"').",
        file=sys.stderr,
    )
    sys.exit(1)
...
def generate_qr(data_to_encode, output_filepath, image_format_str, box_s, border_s):
    ...
    """
    print(
        f"Generating QR code for data (first 50 chars): '{data_to_encode[:50]}{'...' if len(data_to_encode) > 50 else ''}'"
    )
    ...
    try:
        ...
        if image_format_str.lower() == "svg":
            ...
            print(
                f"Error: Unsupported image format '{image_format_str}'. Supported: PNG, JPEG, BMP, GIF, TIFF, SVG.",
                file=sys.stderr,
            )
            return False
        ...
        with open(
            output_filepath, "wb"
        ) as f:  # Open in binary mode for Pillow and SVG factory
            img.save(
                f,
                kind=(
...
if __name__ == "__main__":
    ...
    input_group.add_argument(
        ...
        nargs="?",
        default=None,
        help="Text data to encode in the QR code (if --input-file is not used).",
    )
    input_group.add_argument(
        "--input-file",
        metavar="FILEPATH",
        help="Path to a file containing data to encode.",
    )
    ...
    parser.add_argument(
        ...
        metavar="OUTPUT_FILE",
        required=True,
        help="Path to save the generated QR code image (e.g., my_qr.png, qr.svg).",
    )
    ...
    if not final_image_format:  # If format not specified, infer from output filename
        ...
        if ext:
            ...
            print(
                f"No output file extension or --format given. Defaulting to format: {DEFAULT_OUTPUT_FORMAT.upper()}"
            )
    ...
    if not final_image_format:  # Should not happen if default is set
        print(
            "Error: Could not determine output image format. Please use --format or provide a file extension.",
            file=sys.stderr,
        )
        sys.exit(1)

    success = generate_qr(
        input_data_str, args.output, final_image_format, args.box_size, args.border
    )
