...
def convert_json_to_csv(input_filepath, output_filepath, delimiter=','):
    ...
    try:
        ...
        print(f"Error decoding JSON from '{input_filepath}': {e.msg} (Line: {e.lineno}, Column: {e.colno}).")
        return False
    except IOError as e:
    ...
    if not data:
        print("Warning: JSON array is empty. Output CSV will only contain headers (if any derivable) or be empty.")
        ...
        try:
            with open(output_filepath, 'w', newline='', encoding='utf-8') as csv_file:
                ...
                pass
            print(f"Created an empty CSV file: '{output_filepath}' as JSON array was empty.")
            return True
        except IOError as e:
    ...
    for item in data:
        if not isinstance(item, dict):
            print(f"Warning: Item '{item}' in JSON array is not an object (dictionary). Skipping this item.")
            continue
    ...
    if not headers:
        print("Error: Could not determine any headers from the JSON objects (e.g., all objects were empty or not dictionaries).")
        # Similar to empty data, create an empty CSV.
        try:
            with open(output_filepath, 'w', newline='', encoding='utf-8') as csv_file:
                pass
            print(f"Created an empty CSV file: '{output_filepath}' as no headers could be derived.")
            return True
        except IOError as e:
            print(f"Error writing empty CSV file '{output_filepath}': {e}")
            return False
    ...
    try:
        print(f"Attempting to write CSV data to: '{output_filepath}' with delimiter '{delimiter}'")
        with open(output_filepath, mode='w', newline='', encoding='utf-8') as csv_file:
            ...
            writer = csv.DictWriter(csv_file, fieldnames=headers, delimiter=delimiter, restval='')
...
if __name__ == "__main__":
    ...
    print(f"Using delimiter for CSV: '{delimiter_to_use}' (raw arg: '{args.delimiter}')")

    success = convert_json_to_csv(args.input_json_file, args.output_csv_file, delimiter_to_use)
