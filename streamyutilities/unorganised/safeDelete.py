...
def safe_delete_item(item_path):
    ...
    while os.path.exists(target_trash_path):
        ...
        if len(temp_base_name) > 250:  # Max filename length is often around 255 chars
            ...
            print(
                f"Warning: Generated filename for '{base_name}' is getting very long. Trying timestamp."
            )
            timestamp_suffix = datetime.now().strftime(
                "%Y%m%d%H%M%S%f"
            )  # added microsecs for more uniqueness
            temp_base_name = (
                f"{original_name_part}_{timestamp_suffix}{original_ext_part}"
            )
            target_trash_path = os.path.join(TRASH_FILES_DIR, temp_base_name)
            # If even this collides (extremely unlikely), it will error out or be caught by a re-loop.
            if os.path.exists(target_trash_path):
                print(
                    f"Error: Could not resolve filename conflict for '{base_name}' even with timestamp. Skipping."
                )
                return False
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "paths", nargs="+", help="One or more file or directory paths to move to trash."
    )
