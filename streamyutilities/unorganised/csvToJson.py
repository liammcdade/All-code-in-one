...
def convert_csv_to_json(input_filepath, output_filepath, delimiter=","):
    ...
    try:
        print(
            f"Attempting to read CSV file: '{input_filepath}' with delimiter '{delimiter}'"
        )
        with open(input_filepath, mode="r", newline="", encoding="utf-8") as csv_file:
            ...
            if line_count == 0 and csv_reader.fieldnames is None:
                print(
                    f"Warning: CSV file '{input_filepath}' might be empty or headers not found with delimiter '{delimiter}'."
                )
                ...
                print(
                    f"Warning: CSV file '{input_filepath}' read successfully but contained no data rows after headers."
                )
        ...
        print(
            f"Error parsing CSV file '{input_filepath}': {e}. Check delimiter and file format."
        )
        return False

    if (
        not data_list and not csv_reader.fieldnames
    ):  # Double check if anything was really processed
        ...
        print(
            f"No data processed from CSV. Output JSON will likely be empty or just '[]'."
        )
    ...
    try:
        ...
        TypeError
    ) as e:  # Handles issues if data_list contains non-serializable types
...
if __name__ == "__main__":
    ...
    success = convert_csv_to_json(
        args.input_csv_file, args.output_json_file, delimiter_to_use
    )
