        ...
        if os.path.isdir(target_pattern) and not glob.has_magic(target_pattern):
            ...
            if glob.has_magic(target_pattern):
                ...
                # This part is tricky. For now, we'll glob everything that's not an explicit dir.
                 found_items = glob.glob(target_pattern, recursive=recursive_flag) # Enable if user wants recursion
...
def search_replace_in_file(filepath, search_pattern, replacement_string, is_regex, dry_run, interactive_mode):
    """
    ...
    try:
        ...
        try:
            ...
            print(f"Warning: Could not decode '{filepath}' as UTF-8. Trying with 'latin-1'.")
        ...
        if is_regex:
            try:
                new_content, num_replacements = re.subn(search_pattern, replacement_string, content)
            except re.error as e:
                print(f"Regex error for pattern '{search_pattern}' in '{filepath}': {e}. Skipping file.")
                return 0, False, True
        ...
        if dry_run:
            ...
            return num_replacements, False, False # Report potential changes, but file not actually changed

        if interactive_mode:
            # Per-file confirmation for now. Per-occurrence is more complex.
            user_confirm = input(f"Apply {num_replacements} change(s) to '{filepath}'? (yes/no/all_remaining): ").strip().lower()
            if user_confirm == 'yes':
                ...
                print("Treating 'all_remaining' as 'yes' for this file. Future files will still ask if in interactive mode.")
                pass
        ...
        try:
            with open(filepath, 'w', encoding='utf-8' if 'UnicodeDecodeError' not in locals() else 'latin-1') as f:
                f.write(new_content)
...
if __name__ == "__main__":
    ...
    parser.add_argument("search_pattern", help="The string or regex pattern to search for.")
    parser.add_argument("replacement_string", help="The string to replace occurrences with.")
    parser.add_argument(
        "input_targets",
        nargs="+",
    ...
    for target in args.input_targets:
        if os.path.isdir(target):
            ...
            for g_item in expanded_glob:
                ...
                elif os.path.isdir(g_item) and args.recursive: # If glob matched a dir and we are recursive
