...
def venv_exists(path):
    ...
    if os.path.isdir(path):
        ...
        if os.path.isfile(os.path.join(path, "bin", "activate")) or os.path.isfile(
...
def create_venv(name, python_exe):
    ...
    if venv_exists(target_path):
        print(
            f"Error: Virtual environment '{display_name}' already exists at '{target_path}'.",
            file=sys.stderr,
        )
    ...
    if name and not os.path.exists(VENV_BASE_DIR):
        try:
            ...
            print(
                f"Created base directory for named virtual environments: '{VENV_BASE_DIR}'"
            )
        except OSError as e:
            print(
                f"Error: Could not create base directory '{VENV_BASE_DIR}': {e}",
                file=sys.stderr,
            )
            return
    ...
    print(
        f"Creating virtual environment '{display_name}' at '{target_path}' using '{python_exe}'..."
    )
    ...
    try:
        ...
        print(
            f"An unexpected error occurred during venv creation: {e}", file=sys.stderr
        )
...
def activate_venv_instructions(name):
    ...
    if not venv_exists(target_path):
        print(
            f"Error: Virtual environment '{display_name}' not found at '{target_path}'.",
            file=sys.stderr,
        )
        print(
            f"Consider creating it first: python {os.path.basename(__file__)} create {name if name else ''}",
            file=sys.stderr,
        )
        return

    print(
        f"To activate the virtual environment '{display_name}', run the following command in your shell:"
    )
    ...
    if os.name == "posix":  # Linux, macOS, etc.
        ...
        print(
            "    (If PowerShell script execution is restricted, you might need to run: Set-ExecutionPolicy RemoteSigned -Scope CurrentUser)\n"
        )
    else:
        print(
        ...
        print(
            f"Look for an 'activate' script inside '{os.path.join(target_path, 'bin')}' or '{os.path.join(target_path, 'Scripts')}'.\n"
        )
...
def list_venvs():
    ...
    if not os.path.isdir(VENV_BASE_DIR):
        print(
            "Base directory for named virtual environments does not exist or is not a directory."
        )
...
def remove_venv(name):
    target_path = get_venv_path(name)
    display_name = (
        name if name else f"./{LOCAL_VENV_NAME}"
    )  # Should require name for remote

    if (
        not name
    ):  # Safety: require explicit name for remove, don't just delete ./.venv implicitly
        print(
            "Error: You must specify the name of the virtual environment to remove.",
            file=sys.stderr,
        )
        print(
            "To remove a local './.venv', provide '.' or '.venv' as the name if that's intended (not yet supported, use manual `rm -rf .venv`).",
            file=sys.stderr,
        )
    ...
    if not venv_exists(target_path):  # Checks named venv path
        print(
            f"Error: Named virtual environment '{name}' not found at '{target_path}'.",
            file=sys.stderr,
        )
        return

    print(
        f"WARNING: This will permanently delete the virtual environment '{name}' at '{target_path}'."
    )
    try:
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="Manage Python virtual environments using the 'venv' module."
    )
    subparsers = parser.add_subparsers(
        dest="action", title="Actions", help="Available actions", required=True
    )
    ...
    parser_activate = subparsers.add_parser(
        "activate", help="Show command to activate a virtual environment."
    )
    ...
    parser_list = subparsers.add_parser(
        "list", help="List available managed virtual environments."
    )

    # Remove command
    parser_remove = subparsers.add_parser(
        "remove", help="Remove a named virtual environment."
    )
