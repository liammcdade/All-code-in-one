...
def ensure_trash_structure_exists():
    ...
    if not os.path.isdir(TRASH_FILES_DIR) or not os.path.isfile(TRASH_LOG_FILE):
        print(
            "Trash directory or log file not found. Looks like safeDelete hasn't been used or trash is clean."
        )
        return False
    return True
...
def confirm_deletion(count):
    ...
    try:
        confirm = (
            input(f"Permanently delete {count} item(s) from trash? (yes/no): ")
            .strip()
            .lower()
        )
    except EOFError:
...
def delete_selected_items(items_to_delete_basenames, all_log_entries):
    ...
    for basename in items_to_delete_basenames:
        ...
        try:
            if os.path.exists(
                ...
                if os.path.isfile(item_path_in_trash) or os.path.islink(
                    item_path_in_trash
                ):
                    ...
                    print(
                        f"Warning: '{basename}' is not a file or directory. Skipping FS removal."
                    )
                ...
                print(
                    f"Note: '{basename}' was already gone from trash files directory."
                )

            fs_deleted_successfully_basenames.add(
                basename
            )  # Add if FS deletion succeeded or file was already gone
    ...
    if (
        fs_deleted_successfully_basenames
    ):  # If any FS operations were successful (or files confirmed gone)
        save_trash_log(updated_log_entries)
        print(
            f"Successfully processed {len(fs_deleted_successfully_basenames)} item(s): files removed/confirmed gone from filesystem and log updated."
        )
...
if __name__ == "__main__":
    ...
    group.add_argument(
        "--list", action="store_true", help="List items currently in trash."
    )
    group.add_argument(
        "--all", action="store_true", help="Delete all items from trash."
    )
    group.add_argument(
        "--name",
        metavar="TRASHED_NAME",
        help="Delete a specific item by its name in the trash files directory.",
    )
    group.add_argument(
        "--days", metavar="N", type=int, help="Delete items older than N days."
    )
    ...
    if args.list:
        ...
        if os.path.exists(item_full_path):
            ...
            print(
                f"Item '{args.name}' not found in trash files directory: '{TRASH_FILES_DIR}'."
            )
            # Check if it's only in the log (a discrepancy)
            is_in_log = any(
                os.path.basename(entry.get("trash_path", "")) == args.name
                for entry in current_log_entries
            )
            if is_in_log:
                print(
                    f"Item '{args.name}' found in log, but not in file system. You may want to run with --all or clear specific log entries if this is frequent."
                )
            ...
            try:
                ...
                if deleted_at_dt < cutoff_date:
                    ...
                    if os.path.exists(os.path.join(TRASH_FILES_DIR, basename)):
                        ...
                        print(
                            f"Note: Logged item '{basename}' (original: {entry.get('original_path')}) is older than {args.days} days but already missing from trash files. Will clean its log entry if deletion proceeds."
                        )
                        files_to_delete_basenames.add(
                            basename
                        )  # Add to ensure log cleanup
            except (ValueError, TypeError):
                print(
                    f"Warning: Could not parse deletion date for log entry: {entry.get('trash_path')}. Skipping for age check."
                )
