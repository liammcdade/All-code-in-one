    ...
    try:
        ...
        with urllib.request.urlopen(req, timeout=REQUEST_TIMEOUT) as response:
            ...
            if response.status == 200:
                if short_url.startswith("http://tinyurl.com/") or short_url.startswith("https://tinyurl.com/"):
                    return short_url
                else:
                    # Response might be an error message from TinyURL not sent with an HTTP error code
                    print(f"Error: Service returned an unexpected response: {short_url[:100]}")
                    return None
        ...
        print(f"HTTP Error from service: {e.code} {e.reason}. Response: {error_content[:200]}")
        if e.code == 400:
        ...
        print(f"URL Error: Could not reach the URL shortening service. Reason: {e.reason}")
        return None
    except TimeoutError: # Explicitly catch timeout from urllib.request.urlopen
        print(f"Error: The request to the URL shortening service timed out after {REQUEST_TIMEOUT} seconds.")
        return None
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "long_url",
        help="The long URL that you want to shorten."
    )

    args = parser.parse_args()

    # Basic validation for the input URL structure (very simple check)
    if not (args.long_url.startswith("http://") or args.long_url.startswith("https://")):
        ...
        print("Warning: The provided URL does not seem to have a scheme (http:// or https://).")
        print("Attempting to shorten it as is, but the service might require a full URL including the scheme.")
