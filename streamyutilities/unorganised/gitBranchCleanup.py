...
def get_current_branch_name():
    ...
    stdout, stderr, retcode = run_git_command(
...
def main():
    ...
    print(
        "WARNING: This script will delete local Git branches. Ensure you understand its operation.\n"
    )

    # 1. Check if inside a Git repository
    _, stderr_git_repo, retcode_git_repo = run_git_command(
        ["git", "rev-parse", "--is-inside-work-tree"]
    )
    if (
        retcode_git_repo != 0 or stderr_git_repo == "Not a git repository"
    ):  # Check specific stderr for older git
        ...
        stdout_is_repo, _, _ = run_git_command(
            ["git", "rev-parse", "--is-inside-work-tree"]
        )
    ...
    if actual_current_branch is None or "detached HEAD" in actual_current_branch:
        print(
            f"Currently in a '{actual_current_branch or 'unknown'}' state.",
            file=sys.stderr,
        )
        if args.target_branch:
            ...
            if local_branches_for_default is None:
                return
            if "main" in local_branches_for_default:
                target_branch = "main"
            elif "master" in local_branches_for_default:
                target_branch = "master"
            else:
                print(
                    "Error: Detached HEAD and could not determine a default target branch ('main' or 'master' not found). Please specify --target-branch.",
                    file=sys.stderr,
                )
                return
            print(
                f"Defaulting target branch to '{target_branch}' due to detached HEAD state."
            )
    else:
        print(f"Current branch is: '{actual_current_branch}'")
        target_branch = (
            args.target_branch if args.target_branch else actual_current_branch
        )
    ...
    if all_local_branches is None:
        return
    if target_branch not in all_local_branches and target_branch not in (
        actual_current_branch if actual_current_branch else ""
    ):  # target_branch could be current if it's symbolic like 'HEAD'
        # A more robust check: `git show-ref --verify --quiet refs/heads/{target_branch}`
        _, _, retcode_verify_target = run_git_command(
            ["git", "rev-parse", "--verify", f"{target_branch}^{{commit}}"]
        )  # Check if resolvable to a commit
        if retcode_verify_target != 0:
            print(
                f"Error: Target branch '{target_branch}' does not exist or is not a valid branch.",
                file=sys.stderr,
            )
            return
    ...
    stdout_merged, stderr_merged, retcode_merged = run_git_command(
    ...
    for line in stdout_merged.splitlines():
        ...
        if (
            ...
            and branch_name == actual_current_branch.split(" ")[-1]
        ):
            continue  # Handle "detached HEAD at X"
    ...
    if not merged_branches_candidates:
        print(
            f"No local branches found that are merged into '{target_branch}' and not protected."
        )
        return
    ...
    if args.dry_run:
        ...
        print(
            f"The above {len(merged_branches_candidates)} branch(es) would be targeted for deletion."
        )
        return
    ...
    try:
        action = (
            input(
                "Enter 'all' to delete all listed, 'none' to cancel, or comma/hyphen-separated indices (e.g., 1,3-5) to delete specific branches: "
            )
            .strip()
            .lower()
        )
            ...
            for part in parts:
                ...
                if "-" in part:
                    ...
                    if not (1 <= start <= end <= len(merged_branches_candidates)):
                        raise ValueError(
                            f"Range '{part}' out of bounds (1-{len(merged_branches_candidates)})."
                        )
                    ...
                    if not (1 <= idx <= len(merged_branches_candidates)):
                        raise ValueError(
                            f"Index '{part}' out of bounds (1-{len(merged_branches_candidates)})."
                        )
    ...
    for branch_name in branches_to_delete:
        ...
        stdout_del, stderr_del, retcode_del = run_git_command(
            ["git", "branch", "-d", branch_name]
        )
        if retcode_del == 0:
            # stdout_del often contains "Deleted branch X (was YYY)."
            print(
                f"Success. ({stdout_del or stderr_del or 'OK'})"
            )  # stderr might contain info on successful -d too
            deleted_count += 1
        else:
    ...
    if failed_deletions:
        ...
        print(
            "This might be because they have unmerged changes (use 'git branch -D <branchname>' to force delete) or other issues."
        )
    return
