...
LANGUAGE_DEFINITIONS = {
    ...
    "JavaScript": {
        ...
        "block_comment_start": re.compile(
            r"^\s*/\*"
        ),  # Simplified: line must start with /*
        "block_comment_end": re.compile(
            r"\*/\s*$"
        ),  # Simplified: line must end with */
    },
    "Java": {
        "extensions": [".java"],
        "single_line_comment": re.compile(r"^\s*//"),
    ...
    for line in content_lines:
        ...
        if block_start_re:  # If language supports block comments
            if in_block_comment:
                ...
                if block_end_re and block_end_re.search(
                    line
        ...
        if not is_comment_line and single_line_re:
            if single_line_re.match(
                stripped_line
            ):  # Check if line starts with comment (ignoring whitespace)
...
def analyze_project(target_dir, ignore_patterns_str, output_format):
    ...
    if not os.path.isdir(abs_target_dir):
        print(
            f"Error: Target directory '{abs_target_dir}' not found or is not a directory.",
            file=sys.stderr,
        )
        return
    ...
    ignore_dir_names = {
        p
        for p in raw_ignore_patterns
        if not any(c in p for c in ["*", "?", "["]) and p.endswith("/")
    }
    ...
    for root, dirs, files in os.walk(abs_target_dir, topdown=True):
        ...
        for filename in files:
            ...
            if any(
                fnmatch.fnmatch(filename, file_pat)
                for file_pat in ignore_file_patterns
                if not file_pat.endswith("/")
            ):
                continue
            ...
            if language:
                try:
                    ...
                    print(
                        f"Warning: Could not process file '{filepath}': {e}",
                        file=sys.stderr,
                    )
    ...
    output_data = {
        ...
        "unknown_extensions_details": (
            dict(unknown_extensions) if unknown_extensions else {}
        ),
    }

    for lang, stats in sorted(
        language_stats.items(), key=lambda item: item[1]["loc"], reverse=True
    ):
        percentage = (
            (stats["loc"] / total_loc_all_langs * 100) if total_loc_all_langs > 0 else 0
        )
