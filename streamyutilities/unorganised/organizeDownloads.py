...
DEFAULT_MAPPINGS = {
    ...
    "Documents": [".pdf", ".docx", ".doc", ".txt", ".odt", ".rtf", ".csv", ".xls", ".xlsx", ".ppt", ".pptx"],
...
def organize_downloads(downloads_dir, custom_mapping_file, unknown_action="other", dry_run=False):
    ...
    for filename in os.listdir(downloads_dir):
        ...
        if src_path == dest_path: # Already in the correct place (e.g. in "Other" and unknown)
            continue

        proposed_moves.append((src_path, dest_path, category_folder_name))
        if not os.path.exists(dest_folder_path):
            folders_to_create.add(category_folder_name) # Store just name, relative to downloads_dir
    ...
    for src, dest, _ in proposed_moves:
        print(f"    - Move '{os.path.basename(src)}' to '{os.path.relpath(dest, downloads_dir)}'")
        ...
        for src_path, dest_path, _ in proposed_moves:
            try:
                ...
                if not os.path.exists(dest_dir):
                     ...
                     print(f"Warning: Destination directory '{dest_dir}' for '{os.path.basename(src_path)}' does not exist. Skipping.")
                     continue

                if os.path.exists(dest_path):
                    print(f"Warning: File '{dest_path}' already exists. Skipping '{os.path.basename(src_path)}'.")
                    continue

                shutil.move(src_path, dest_path)
                moved_count += 1
            except Exception as e:
                print(f"Error moving '{os.path.basename(src_path)}' to '{dest_path}': {e}")
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "downloads_dir",
        help="The directory whose files need to be organized."
    )
    parser.add_argument(
        "--custom-mapping",
