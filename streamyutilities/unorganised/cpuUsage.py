...
if __name__ == "__main__":
    ...
    print(
        f"Starting CPU usage monitoring (Interval: {args.interval}s, Count: {'Infinite' if args.count == 0 else args.count})."
    )
    print("Press Ctrl+C to stop.")
    print(
        "Note: The first reading for overall CPU usage might be 0.0% as psutil establishes a baseline."
    )
    ...
    try:
        ...
        while True:
            ...
            if overall_usage is None:  # Should ideally not happen if psutil is working
                print(
                    "Warning: Could not retrieve overall CPU usage for this interval.",
                    file=sys.stderr,
                )
            ...
            if overall_usage is None:  # If psutil call actually returns None
                print(
                    "Overall CPU usage measurement failed for this interval.",
                    file=sys.stderr,
                )
                ...
                ):  # check again in case this was the last
                    break
