...
def get_archive_destination_path(source_dir, custom_dest_dir=None):
    ...
    if custom_dest_dir:
        ...
        proposed_dest = os.path.join(
            os.path.abspath(source_dir), archive_subfolder_name
        )
        # As a fallback, use user's Documents folder
        fallback_dest_base = os.path.join(
            os.path.expanduser("~"), "Documents", "ArchivedFiles"
        )
        ...
        try:
            ...
            print(
                f"Warning: Default destination '{proposed_dest}' might not be usable."
            )
...
def find_old_files(source_directory, days_old, recursive):
    ...
    if recursive:
        for root, _, files in os.walk(abs_source_directory):
            ...
            if (
                "archive_" in os.path.basename(root)
                and os.path.dirname(root) == abs_source_directory
            ):  # simple check
                continue
                    ...
                    if os.path.isfile(filepath) and not os.path.islink(filepath):
                        ...
                        if file_mod_time < cutoff_time:
                            ...
                            relative_path = os.path.relpath(
                                filepath, abs_source_directory
                            )
                            old_files_map[filepath] = relative_path
                except OSError as e:
                    print(
                        f"Warning: Could not access or get info for '{filepath}': {e}"
                    )
    else:  # Non-recursive
...
def archive_files(
    source_dir_path, days, dest_dir_path=None, recursive_scan=False, dry_run_mode=False
):

    archive_root_dest = get_archive_destination_path(source_dir_path, dest_dir_path)

    print(
        f"Scanning for files older than {days} days in '{os.path.abspath(source_dir_path)}'."
    )
    if recursive_scan:
    ...
    if archive_root_dest.startswith(
        abs_source_dir
    ) and abs_source_dir != os.path.dirname(archive_root_dest):
    ...
    if confirm == "yes":
        ...
        try:
            ...
            print(
                f"Error: Could not create archive base directory '{archive_root_dest}': {e}. Aborting."
            )
            return
            ...
            try:
                ...
                if os.path.exists(final_dest_path):
                    ...
                    final_dest_path = os.path.join(
                        final_dest_subdir, f"{name}{conflict_suffix}{ext}"
                    )
                    print(
                        f"Warning: Target '{os.path.join(archive_root_dest, dest_rel_path)}' existed. Saving as '{final_dest_path}'."
                    )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        type=int,
        required=True,
        help="Files older than this number of days will be archived.",
    )
    parser.add_argument(
        "--destination",
        help="Optional: Directory where archives will be stored. Defaults to 'archive_[YYYYMMDD]' in source_dir or ~/Documents/ArchivedFiles.",
    )
    parser.add_argument(
        "--recursive",
        action="store_true",
    ...
    archive_files(
        args.source_directory, args.days, args.destination, args.recursive, args.dry_run
    )
