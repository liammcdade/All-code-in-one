...
def check_tcp_port(host, port, timeout):
    ...
    try:
        ...
        if result_code == 0:
            ...
            error_reason = (
                os.strerror(result_code)
                if hasattr(os, "strerror") and result_code != 0
                else f"Error code {result_code}"
            )
            return (
                False,
                f"Port {port} on host '{host}' is closed or unreachable. Reason: {error_reason}",
            )

    except (
        socket.timeout
    ):  # This can happen if the host is up but the port is filtered, or host is slow
...
if __name__ == "__main__":
    ...
    parser.add_argument("port", type=int, help="The port number to check (1-65535).")
    parser.add_argument(
        "--hostname",
        default="localhost",
    ...
    if not (1 <= args.port <= 65535):
        print(
            f"Error: Port number {args.port} is invalid. Must be between 1 and 65535."
        )
        return
    ...
    print(
        f"Attempting to connect to host '{args.hostname}' on port {args.port} with a timeout of {args.timeout}s..."
    )
