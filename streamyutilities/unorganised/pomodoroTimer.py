...
if __name__ == "__main__":
    ...
    if any(
        arg_val <= 0
        for arg_val in [args.work, args.short_break, args.long_break, args.sessions]
    ):
        print(
            "Error: All duration and session count arguments must be positive integers.",
            file=sys.stderr,
        )
        sys.exit(1)
    ...
    print(
        f"Work: {args.work} min | Short Break: {args.short_break} min | Long Break: {args.long_break} min | Sessions for Long Break: {sessions_for_long_break}"
    )
    ...
    try:
        while True:
            ...
            play_notification(
                f"Work session {work_session_counter} finished! Time for a break."
            )

            # Break session
            if (
                work_session_counter > 0
                and work_session_counter % sessions_for_long_break == 0
            ):
                run_timer_session(long_break_sec, "Long Break")
                play_notification(
                    "Long break finished! Time for the next work session."
                )
            else:
                run_timer_session(short_break_sec, "Short Break")
                play_notification(
                    "Short break finished! Time for the next work session."
                )
