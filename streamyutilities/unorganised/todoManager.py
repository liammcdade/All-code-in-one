...
def load_tasks():
    ...
    try:
        ...
        if not isinstance(tasks, list) or not all(isinstance(t, dict) for t in tasks):
            print(
                f"Warning: Data in '{TODO_FILE_PATH}' is not in the expected format (list of tasks). Starting fresh if modified.",
                file=sys.stderr,
            )
        ...
        print(
            f"Warning: Could not decode JSON from '{TODO_FILE_PATH}'. File might be corrupted.",
            file=sys.stderr,
        )
        return []  # Or raise
    except IOError as e:
...
def list_tasks(
    status_filter=None, show_ids=True
):  # Default show_ids to True as per example
    tasks = load_tasks()
    if not tasks:
    ...
    for task in filtered_tasks:
        ...
        print(
            f"{id_prefix}{status_marker} {desc} {created_display} {completed_display}".strip()
        )
    if not filtered_tasks and (
        status_filter and status_filter != "all"
    ):  # Redundant due to check above
        pass
    ...
    for task in tasks:
        if task.get("id") == task_id:
            ...
            if task["status"] == "completed":
                print(
                    f"Task {task_id} \"{task['description']}\" is already marked as completed."
                )
            else:
                ...
                if save_tasks(tasks):
                    print(
                        f"Marked task {task_id} \"{task['description']}\" as completed."
                    )
                else:
...
def edit_task(task_id, new_description):
    ...
    for task in tasks:
        if task.get("id") == task_id:
            ...
            if save_tasks(tasks):
                print(
                    f'Edited task {task_id}: from "{old_description}" to "{new_description}"'
                )
            else:
...
if __name__ == "__main__":
    ...
    parser_done.add_argument(
        "task_id", type=int, help="The ID of the task to mark as completed."
    )
    ...
    parser_edit = subparsers.add_parser(
        "edit", help="Edit an existing task's description."
    )
