...
if __name__ == "__main__":
    ...
    if not os.path.isdir(serve_directory):
        print(
            f"Error: Directory '{serve_directory}' not found or is not a directory.",
            file=sys.stderr,
        )
        sys.exit(1)
    ...
    try:
        ...
        TypeError
    ):  # 'directory' keyword unavailable (Python < 3.7) or other init issue
        print(
            "Warning: functools.partial with directory failed (Python < 3.7 or other issue)."
        )
        print(
            f"Attempting to serve by changing current directory to '{serve_directory}'."
        )
        try:
            ...
            print(
                f"Error: Could not change to directory '{serve_directory}': {e_chdir}",
                file=sys.stderr,
            )
            sys.exit(1)
    ...
    try:
        ...
        if e.errno == 98:  # Address already in use [Errno 98]
            print(
                f"\nError: Port {args.port} on address '{args.bind}' is already in use.",
                file=sys.stderr,
            )
        else:
