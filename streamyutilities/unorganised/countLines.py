...
def count_lines_in_file(filepath, ignore_comments, ignore_blank_lines):
    ...
    block_comment_start_re, block_comment_end_re = BLOCK_COMMENT_PATTERNS.get(file_ext, (None, None))
    ...
    try:
        with open(filepath, 'r', encoding='utf-8', errors='ignore') as f:
            for line_content in f:
                ...
                if current_line_is_loc and ignore_comments: # Only check for comments if it's not already disqualified
                    line_is_comment = False
                    # Test for block comments first
                    if block_comment_start_re and block_comment_end_re:
                        # Case 1: Already in a block comment
                        if in_block_comment:
                            if block_comment_end_re.search(line_content): # End of block comment?
                        ...
                        # Case 2: Not in a block comment, check for start
                            ...
                            if block_comment_end_re.search(line_content): # Ends on the same line
                                in_block_comment = False
...
if __name__ == "__main__":
    ...
    parser.add_argument("filepaths", nargs="+", help="One or more file paths to analyze.")
    parser.add_argument(
        "--ignore-comments",
        action="store_true",
    ...
    for filepath in args.filepaths:
        ...
        file_stats = count_lines_in_file(filepath, args.ignore_comments, args.ignore_blank_lines)
