...
def process_base64(mode, input_data_bytes=None, output_filepath=None):
    ...
    if output_filepath:
        try:
            ...
            print(
                f"An unexpected error occurred while writing to '{output_filepath}': {e}"
            )
            return False
        ...
        if mode == "encode":
            ...
            try:
                ...
                print(
                    "Decoded output (likely binary, showing partial representation or info):"
                )
...
if __name__ == "__main__":
    ...
    input_group.add_argument(
        "--string", metavar="TEXT", help="Input string for encoding/decoding."
    )
    input_group.add_argument(
        "--input-file",
        metavar="FILEPATH",
        help="Path to input file for encoding/decoding.",
    )

    parser.add_argument(
        "--output-file", metavar="FILEPATH", help="Optional: Path to output file."
    )
    ...
    if args.string is not None:
        print(
            f"Input from string: '{args.string[:30]}{'...' if len(args.string)>30 else ''}'"
        )
        if args.mode == "encode":
        ...
        try:
            ...
            print(
                f"An unexpected error occurred while reading input file '{args.input_file}': {e}"
            )
            exit(1)

    if (
        input_bytes_to_process is None and args.mode == "encode"
    ):  # Should not happen if string or file is required
