...
def find_processes_by_name(name_pattern, case_sensitive=False):
    ...
    for proc in psutil.process_iter(attrs=attrs, ad_value=None): # ad_value=None to skip on AccessDenied
        ...
        if case_sensitive:
            ...
            if search_term in proc_name.lower() or search_term in proc_cmdline_str.lower():
                match_found = True
...
def kill_process(proc, use_sigkill=False):
    ...
    try:
        ...
        print(f"Access denied to send signal to PID {pid} ({name}). Try with higher privileges.")
        return False
...
if __name__ == "__main__":
    ...
    parser.add_argument("process_name_pattern", help="Name or part of the name/command line of the process(es) to find.")
    parser.add_argument(
        "--case-sensitive",
        action="store_true",
    ...
    print(f"Searching for processes matching: '{args.process_name_pattern}' (Case sensitive: {args.case_sensitive})")

    try:
        matched_procs = find_processes_by_name(args.process_name_pattern, args.case_sensitive)
    except Exception as e:
    ...
    for i, proc in enumerate(matched_procs):
        try:
            ...
            print(f"  {i+1}. PID: {proc.pid:<6} User: {username:<15} Status: {status:<10} Name: {proc.info.get('name', 'N/A'):<20} Cmd: {cmdline[:60]}{'...' if len(cmdline)>60 else ''}")
        except psutil.NoSuchProcess: # Process might have ended since initial listing
             print(f"  {i+1}. PID: {proc.pid:<6} (Process ended before details could be fully retrieved)")
        except psutil.AccessDenied:
             print(f"  {i+1}. PID: {proc.pid:<6} (Access denied to full details for this process)")
    ...
    for proc_obj in procs_to_kill:
        ...
        if kill_process(proc_obj, args.force):
            killed_count += 1 # Count successful signal sends, not necessarily successful kills

    print(f"\nFinished sending signals. {killed_count} signal(s) were sent successfully.")
    print("Note: 'Signal sent' does not guarantee the process terminated, especially for SIGTERM.")
    print("You may need to re-run the search or use system tools to verify termination.")
    sys.exit(0)
