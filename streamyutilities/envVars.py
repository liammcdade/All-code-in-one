...
def format_variables(variables_dict, output_format):
    ...
    if not variables_dict:
        return ["No environment variables match the criteria or none are set (unlikely)."]
    ...
    if output_format == 'list':
        ...
        try:
            ...
            output_lines.append(f"Error: Could not serialize environment variables to JSON: {e}")
...
if __name__ == "__main__":
    ...
    if args.name:
        ...
        var_value = os.getenv(args.name) # os.getenv returns None if not found, vs os.environ[] raising KeyError
        if var_value is not None:
            ...
            if args.format == 'export':
                ...
                print(var_value) # Simple value print for --name as per common expectation
        else:
