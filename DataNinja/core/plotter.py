...
class DataPlotter:
    ...
    def __init__(self, data):
        ...
        if isinstance(data, pd.DataFrame):
            ...
            try:
                ...
                raise ValueError(f"Could not convert list to DataFrame for plotting: {e}")
        else:
            raise TypeError("Unsupported data type. Please provide a pandas DataFrame or a convertible list.")
    ...
    def create_plot(self, plot_type, **kwargs):
        ...
        if hasattr(self, plot_method_name) and callable(getattr(self, plot_method_name)):
            ...
            try:
                ...
                if fig: # Some plot methods might handle show/save internally if complex
                    if save_path:
            ...
            print(f"Warning: Plot type '{plot_type}' is not supported or method '{plot_method_name}' not found.")
            return None

    def plot_histogram(self, column, title=None, xlabel=None, ylabel="Frequency", bins=10, **kwargs):
        """
        ...
        if not pd.api.types.is_numeric_dtype(self.data[column]):
            print(f"Warning: Column '{column}' is not numeric. Histogram may not be meaningful.")
            # Potentially try to convert or handle, but for now, just warn.

        fig, ax = plt.subplots()
        sns.histplot(data=self.data, x=column, bins=bins, kde=kwargs.pop('kde', False), ax=ax, **kwargs)
    ...
    def plot_scatter(self, x_column, y_column, title=None, xlabel=None, ylabel=None, hue=None, **kwargs):
        """
        ...
        if x_column not in self.data.columns or y_column not in self.data.columns:
            print(f"One or both columns ('{x_column}', '{y_column}') not found in data.")
            return None
        ...
        fig, ax = plt.subplots()
        sns.scatterplot(data=self.data, x=x_column, y=y_column, hue=hue, ax=ax, **kwargs)
    ...
    def plot_bar(self, x_column, y_column, title=None, xlabel=None, ylabel=None, estimator='mean', **kwargs):
        """
        ...
        if estimator == 'count':
            ...
            if not pd.api.types.is_numeric_dtype(self.data[y_column]):
                 print(f"Warning: y_column '{y_column}' must be numeric for estimators like 'mean' or 'sum'.")
            ...
            sns.barplot(data=self.data, x=x_column, y=y_column, estimator=estimator, ax=ax, **kwargs)
            ax.set_ylabel(ylabel if ylabel else f"{estimator.capitalize()} of {y_column}")
...
if __name__ == '__main__':
    ...
    sample_data_plot = pd.DataFrame({
        ...
        'Salary': [50000, 60000, 45000, 75000, 58000, 90000, 62000, 48000, 70000, 60000],
        'Department': ['HR', 'Engineering', 'Marketing', 'Engineering', 'HR',
                       'Management', 'Engineering', 'Marketing', 'HR', 'Engineering'],
    ...
    plotter.create_plot(plot_type='histogram', column='Age', bins=8, title="Distribution of Age", kde=True, save_path="age_histogram.png")
    ...
    plotter.create_plot(plot_type='scatter', x_column='Experience', y_column='Salary', hue='Department',
                        title="Salary vs. Experience", save_path="salary_vs_experience.png")
    ...
    plotter.create_plot(plot_type='bar', x_column='Department', y_column='Salary',
                        title="Average Salary by Department", estimator='mean', save_path="avg_salary_by_dept.png")
    print("Bar chart for Average Salary by Department saved to avg_salary_by_dept.png.")

    print("\n--- Plotting Bar Chart (Count by Department) ---")
    plotter.create_plot(plot_type='bar', x_column='Department', y_column='Experience', # y_column is ignored for count
                        title="Employee Count by Department", estimator='count', save_path="count_by_dept.png")
    print("Bar chart for Count by Department saved to count_by_dept.png.")

    print("\n--- Attempting to plot non-existent column ---")
    plotter.create_plot(plot_type='histogram', column='NonExistent', save_path="should_not_save.png")
