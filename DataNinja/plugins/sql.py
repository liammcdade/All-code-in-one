...
class SQLProcessor:
    ...
    def connect(self):
        ...
        self.logger.info(f"Attempting to connect to {self.db_type} database: {self.db_connection_string}")
        ...
        if self.db_type == 'sqlite':
            ...
            self.logger.warning(f"Database type '{self.db_type}' is not yet supported. Only 'sqlite' is available.")
    ...
    def execute_query(self, query: str, params=None, fetch_results=True):
        ...
        self.logger.debug(f"Executing query: {query} with params: {params}, fetch_results: {fetch_results}")
        cursor = None
        try:
            ...
            if fetch_results:
                ...
                self.logger.info(f"Query executed successfully. Fetched {len(df)} rows.")
                return df
            else:
                self.connection.commit()
                rowcount = cursor.rowcount
                self.logger.info(f"DML/DDL query executed successfully. Affected rows: {rowcount if rowcount != -1 else 'N/A'}")
                return rowcount if rowcount != -1 else True # Return True for DDL that don't have rowcount
        except sqlite3.Error as e:
            ...
            raise
        except pd.errors.PandasError as pe: # Catch pandas specific errors during DataFrame creation
            self.logger.error(f"Pandas error during DataFrame creation from query results: {pe}")
            raise
        finally:
            if cursor:
                cursor.close()
    ...
    def execute_script(self, sql_script: str):
        ...
        """
        if not self.connection:
            self.logger.info("No active connection for script execution. Attempting to auto-connect.")
            self.connect()
            if not self.connection:
                 self.logger.error("Failed to establish a database connection for script execution.")
                 raise RuntimeError("Database connection is not active for script execution.")
...
if __name__ == '__main__':
    ...
    try:
        ...
        create_table_query = "CREATE TABLE users (id INTEGER PRIMARY KEY, name TEXT, email TEXT UNIQUE)"
        create_result = processor.execute_query(create_table_query, fetch_results=False)
        main_exec_logger.info(f"CREATE TABLE result: {create_result} (rowcount may be -1 or 0 for DDL)")
        ...
        affected_rows1 = processor.execute_query(insert_query, params=insert_params1, fetch_results=False)
        main_exec_logger.info(f"INSERT 1 affected rows: {affected_rows1}")
        affected_rows2 = processor.execute_query(insert_query, params=insert_params2, fetch_results=False)
        ...
        df_specific = processor.execute_query(select_specific_query, params=('bob@example.com',))
        ...
        try:
            ...
            main_exec_logger.info(f"Caught expected sqlite3.Error for invalid syntax: {e}")

        main_exec_logger.info("Attempting query on non-existent table...")
        try:
            processor.execute_query("SELECT * FROM non_existent_table")
        except sqlite3.Error as e:
            main_exec_logger.info(f"Caught expected sqlite3.Error for non-existent table: {e}")

        main_exec_logger.info("Attempting to connect with unsupported DB type...")
        unsupported_processor = SQLProcessor(db_type='mysql', db_connection_string='dummy')
        try:
        ...
        main_exec_logger.error(f"An error occurred during SQLProcessor tests: {e}", exc_info=True)
    finally:
        # 6. Disconnect
        main_exec_logger.info("\n--- Testing Disconnect ---")
