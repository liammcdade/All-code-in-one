...
class YAMLHandler(DataLoader):
    ...
    def load_data(self, **kwargs):
        ...
        try:
            ...
            self.logger.info(f"Successfully loaded YAML data from {self.source}. Type: {type(data)}")
            return data
            ...
            self.logger.error(f"An unexpected error occurred while loading YAML from {self.source}: {e}")
            raise
    ...
    def save_data(self, data, target_path=None, **kwargs):
        ...
        try:
            with open(target_path, 'w', encoding=encoding) as stream:
                yaml.dump(data, stream, sort_keys=sort_keys, allow_unicode=allow_unicode, **kwargs)
            self.logger.info(f"Successfully saved YAML data to {target_path}")
        except IOError as e:
            self.logger.error(f"IOError occurred while saving YAML data to {target_path}: {e}")
            raise
        except yaml.YAMLError as ye: # Error during dumping
            self.logger.error(f"YAML formatting error while saving to {target_path}: {ye}")
            raise
        except TypeError as te: # Data not serializable
             self.logger.error(f"TypeError: Data not serializable to YAML for {target_path}. {te}")
             raise
        except Exception as e:
            self.logger.error(f"An unexpected error occurred while saving YAML data to {target_path}: {e}")
            raise
...
if __name__ == '__main__':
    ...
    with open(malformed_yaml_path, 'w') as f:
        f.write("project: DataNinja\nversion: 1.2\n  tags: [data, yaml, python]\nusers: - id: 1") # Bad indentation for tags
    ...
    handler_non_existent = YAMLHandler(source=os.path.join(temp_dir, 'non_existent.yaml'))
    try:
    ...
    try:
        ...
        main_logger.error(f"Caught unexpected error for malformed file: {e}", exc_info=True)

    # --- Test save_data ---
    dict_to_save = {'name': 'Test Report', 'score': 85.5, 'details': {'items': 5, 'valid': True}}
    list_to_save = [{'task': 'Setup', 'status': 'Done'}, {'task': 'Run', 'status': 'Pending'}]

    handler_save = YAMLHandler(source="dummy_source_for_saving.yaml") # Source not used for saving
    ...
    try:
        handler_save.save_data(non_serializable_data, target_path=os.path.join(temp_dir,"unsupported.yaml"))
    except yaml.YAMLError as e: # PyYAML raises YAMLError (specifically RepresenterError) for non-serializable
        ...
        main_logger.error(f"Caught unexpected error (non-serializable type): {e}", exc_info=True)
    ...
    try:
        paths_to_remove = [
            sample_yaml_path, malformed_yaml_path, output_dict_yaml_path, output_list_yaml_path,
