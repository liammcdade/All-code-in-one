...
class JSONHandler(DataLoader):
    ...
    def load_data(self, **kwargs):
        ...
        try:
            ...
            self.logger.info(
                f"Successfully loaded JSON data into DataFrame from {self.source}. Shape: {df.shape}"
            )
            return df
        except (
            ValueError
        ) as ve:  # pandas raises ValueError for many JSON parsing issues
            self.logger.warning(
                f"Pandas failed to parse JSON from {self.source} directly into DataFrame: {ve}. Attempting with json.load."
            )
            try:
                ...
                with open(
                    self.source, "r", encoding=kwargs.get("encoding", "utf-8")
                ) as f:  # use encoding from kwargs if present
                    data = json.load(f, **json_load_kwargs)
                self.logger.info(
                    f"Successfully loaded JSON data as {type(data)} from {self.source}"
                )
                return data
                ...
                self.logger.error(
                    f"An unexpected error occurred with json.load for {self.source}: {e_json_load}"
                )
                raise
        except Exception as e_pd:
            self.logger.error(
                f"An unexpected error occurred with pd.read_json for {self.source}: {e_pd}"
            )
            raise
    ...
    def save_data(self, data, target_path=None, **kwargs):
        ...
        if (
            target_dir
        ):  # Ensure target_dir is not an empty string (e.g. if target_path is just a filename)
        ...
        try:
            if isinstance(data, pd.DataFrame):
                ...
                data.to_json(
                    target_path, orient=orient, lines=lines, indent=indent, **kwargs
                )
                self.logger.info(
                    f"Successfully saved pandas DataFrame to JSON: {target_path} (orient='{orient}', lines={lines}, indent={indent})"
                )
            elif isinstance(data, (dict, list)):
                indent = kwargs.pop(
                    "indent", 4
                )  # Default to pretty-printing for dict/list
                with open(
                    target_path, "w", encoding=kwargs.get("encoding", "utf-8")
                ) as f:
                    json.dump(data, f, indent=indent, **kwargs)
                self.logger.info(
                    f"Successfully saved dict/list to JSON: {target_path} (indent={indent})"
                )
            else:
                self.logger.error(
                    f"Unsupported data type for saving to JSON: {type(data)}. Must be DataFrame, dict, or list."
                )
                raise TypeError(
                    f"Unsupported data type for saving to JSON: {type(data)}"
                )
        except IOError as e:
            self.logger.error(
            ...
            TypeError
        ) as e:  # e.g. if data contains non-serializable types for json.dump
            self.logger.error(
                f"TypeError during JSON serialization for {target_path}: {e}"
            )
            raise
...
if __name__ == "__main__":
    ...
    main_logger.setLevel(
        logging.DEBUG
    )  # Ensure this specific logger is at DEBUG if needed.
    ...
    sample_df_json_path = os.path.join(
        temp_dir, "sample_df.json"
    )  # For DataFrame-like JSON (list of records)
    sample_obj_json_path = os.path.join(
        temp_dir, "sample_object.json"
    )  # For dict-like JSON
    ...
    main_logger.info(
        f"Created DataFrame-like JSON (list of records): {sample_df_json_path}"
    )
    ...
    with open(malformed_json_path, "w") as f:
        f.write(
            '{"name": "test", "value": 123,}'
        )  # Trailing comma, but valid JSON for json.load with Python 3.9+
    ...
    handler_non_existent = JSONHandler(
        source=os.path.join(temp_dir, "non_existent.json")
    )
    try:
    ...
    try:
        ...
        ValueError
    ) as e:  # Or ValueError from pandas if it doesn't fallback for some reason
        main_logger.error(
            f"Caught ValueError for malformed JSON (possibly from pandas): {e}"
        )
    except Exception as e:
        main_logger.error(
            f"Caught unexpected error for malformed JSON: {e}", exc_info=True
        )
    ...
    try:
        ...
        generic_handler.save_data(
            df_to_save, target_path=output_df_jsonl_path, orient="records", lines=True
        )
        ...
        if os.path.exists(output_df_jsonl_path):
            ...
            with open(output_df_jsonl_path, "r") as f:
    ...
    try:
        generic_handler.save_data(
            df_to_save,
            target_path=output_df_json_path,
            orient="table",
            indent=4,
            lines=False,
        )
        ...
        if os.path.exists(output_df_json_path):
            main_logger.info(
                f"Verified: {output_df_json_path} exists. First 100 chars:"
            )
            with open(output_df_json_path, "r") as f:
                print(f.read(100) + "...")
    except Exception as e:
        main_logger.error(
            f"Error saving DataFrame to standard JSON: {e}", exc_info=True
        )

    main_logger.info("\n--- Testing save_data (Dict to JSON - indent=4) ---")
    try:
        generic_handler.save_data(
            dict_to_save, target_path=output_dict_json_path
        )  # Uses default indent=4
        main_logger.info(f"Dict saved to JSON: {output_dict_json_path}")
        if os.path.exists(output_dict_json_path):
            main_logger.info(
                f"Verified: {output_dict_json_path} exists. First 100 chars:"
            )
            with open(output_dict_json_path, "r") as f:
                print(f.read(100) + "...")
    except Exception as e:
    ...
    try:
        generic_handler.save_data(
            set([1, 2, 3]), target_path=os.path.join(temp_dir, "unsupported.json")
        )
    ...
    try:
        paths_to_remove = [
            sample_df_json_path,
            sample_obj_json_path,
            malformed_json_path,
            output_df_jsonl_path,
            output_df_json_path,
            output_dict_json_path,
        ...
        if os.path.exists(temp_dir) and not os.listdir(
            temp_dir
