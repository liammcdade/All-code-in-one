...
class SQLiteHandler(DataLoader):
    ...
    def load_data(self, table_name=None, query=None, **kwargs):
        ...
        if (table_name is None and query is None):
            self.logger.error("Either 'table_name' or 'query' must be provided to load_data.")
        ...
        if table_name:
            ...
            if not table_name.replace('_', '').isalnum():
                 self.logger.error(f"Invalid table_name: '{table_name}'. Should be alphanumeric with underscores.")
        ...
        try:
            ...
            self.logger.info(f"Successfully loaded data from {self.source}. Shape: {df.shape}")
            return df
        except sqlite3.OperationalError as e: # Often indicates table not found, SQL error
            self.logger.error(f"SQLite operational error (e.g., table not found, SQL syntax): {e} on query: {sql}")
            raise # Re-raise as it's a significant issue
        except Exception as e:
            self.logger.error(f"An unexpected error occurred while loading data from {self.source}: {e} on query: {sql}")
            raise
        finally:
            if conn:
                conn.close()
    ...
    def save_data(self, data, table_name, if_exists='fail', index=False, **kwargs):
        self.logger.info(f"Attempting to save data to table '{table_name}' in SQLite database: {self.source}")

        if not self.source:
            self.logger.error("Database source path is None or empty.")
            raise ValueError("Database source path is None or empty.")
        if not isinstance(data, pd.DataFrame):
            self.logger.error(f"Data to save must be a pandas DataFrame. Got: {type(data)}")
            raise TypeError(f"Data to save must be a pandas DataFrame. Got: {type(data)}")
        ...
        try:
            ...
            self.logger.info(f"Successfully saved DataFrame to table '{table_name}' in {self.source} (if_exists='{if_exists}').")
        except ValueError as ve: # e.g. if data is empty and table structure cannot be inferred
            self.logger.error(f"ValueError during save_data to table '{table_name}': {ve}")
            raise
        except sqlite3.OperationalError as e:
            self.logger.error(f"SQLite operational error during save_data to table '{table_name}': {e}")
            raise
        except Exception as e:
            self.logger.error(f"An unexpected error occurred while saving data to table '{table_name}' in {self.source}: {e}")
            raise
        finally:
            if conn:
                conn.close()
...
if __name__ == '__main__':
    ...
    df_to_save1 = pd.DataFrame({'id': [1, 2], 'name': ['Alice', 'Bob'], 'value': [10.1, 20.2]})
    df_to_save2 = pd.DataFrame({'id': [3, 4], 'name': ['Charlie', 'David'], 'value': [30.3, 40.4]})
    df_to_save3 = pd.DataFrame({'id': [5, 6], 'name': ['Eve', 'Frank'], 'value': [50.5, 60.6]})
    ...
    try:
        sqlite_handler.save_data(df_to_save1, table_name='test_table', if_exists='fail')
    except ValueError as e: # pandas to_sql raises ValueError if table exists and if_exists='fail'
        main_logger.info(f"Caught expected ValueError for save 'fail' (2nd time): {e}")
    except Exception as e:
        main_logger.error(f"Caught unexpected error for save 'fail' (2nd time): {e}", exc_info=True)

    main_logger.info("\n--- Testing save_data (if_exists='replace') ---")
    try:
        sqlite_handler.save_data(df_to_save2, table_name='test_table', if_exists='replace')
        ...
        assert len(df_after_replace) == len(df_to_save2), "Length mismatch after replace"
        assert df_after_replace['name'].iloc[0] == 'Charlie', "Content mismatch after replace"
        main_logger.info(f"Loaded after replace (len {len(df_after_replace)}): Head:\n{df_after_replace.head()}")
    except Exception as e:
        main_logger.error(f"Error in save 'replace': {e}", exc_info=True)

    main_logger.info("\n--- Testing save_data (if_exists='append') ---")
    try:
        sqlite_handler.save_data(df_to_save3, table_name='test_table', if_exists='append')
        ...
        main_logger.info(f"Loaded after append (len {len(df_after_append)}): Head:\n{df_after_append.head()}")
    ...
    except (sqlite3.OperationalError, pd.errors.DatabaseError) as e: # Pandas may wrap sqlite3.OperationalError
        main_logger.info(f"Caught expected error for non-existent table: {e}")
    except Exception as e:
        main_logger.error(f"Caught unexpected error for non-existent table: {e}", exc_info=True)

    main_logger.info("\n--- Testing load_data (invalid query) ---")
    try:
        sqlite_handler.load_data(query="SELECT FROM table_that_does_not_matter") # Invalid SQL
    except (sqlite3.OperationalError, pd.errors.DatabaseError) as e: # Pandas may wrap sqlite3.OperationalError
        main_logger.info(f"Caught expected error for invalid query: {e}")
    except Exception as e:
        main_logger.error(f"Caught unexpected error for invalid query: {e}", exc_info=True)
    ...
    try:
        sqlite_handler.load_data(table_name="test_table", query="SELECT * FROM test_table")
    except ValueError as e:
        main_logger.error(f"Caught expected ValueError: {e}")
    except Exception as e:
        main_logger.error(f"Caught unexpected error: {e}", exc_info=True)
    ...
    try:
        sqlite_handler.save_data(empty_df, table_name='empty_table_test', if_exists='fail')
        ...
        main_logger.info(f"Loaded 'empty_table_test' (should be empty). Shape: {empty_table_df.shape}")
