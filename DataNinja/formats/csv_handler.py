...
class CSVHandler(DataLoader):
    ...
    def load_data(self, **kwargs):
        ...
        try:
            ...
            self.logger.info(f"Successfully loaded CSV data from {self.source}. Shape: {df.shape}")
            return df
            ...
            self.logger.error(f"An unexpected error occurred while loading {self.source}: {e}")
    ...
    def save_data(self, data, target_path=None, **kwargs):
        ...
        try:
            ...
            data.to_csv(target_path, index=False, **kwargs) # index=False is a common default
...
if __name__ == '__main__':
    ...
    main_logger = logging.getLogger(__name__) # Gets a logger named after the current module
    ...
    try:
        ...
        main_logger.info(f"Loaded DataFrame from empty file. Is empty: {empty_df.empty}. Shape: {empty_df.shape}")
        print(empty_df)
    except Exception as e:
    ...
    try:
        csv_handler_valid.save_data([1,2,3,"not a DataFrame"], target_path=os.path.join(temp_dir,'invalid_output.csv'))
    ...
    try:
        ...
        if os.path.exists(temp_dir) and not os.listdir(temp_dir):
            ...
            main_logger.info(f"Cleaned up: removed directory {temp_dir}")
             ...
             main_logger.warning(f"Directory {temp_dir} not empty, did not remove. Check for leftover files.")
