...
class ExcelHandler(DataLoader):
    ...
    def load_data(self, sheet_name=0, **kwargs):
        self.logger.info(f"Attempting to load Excel data from {self.source}, sheet_name: '{sheet_name}'")
        if not self.source:
        ...
        try:
            ...
            data = pd.read_excel(self.source, sheet_name=sheet_name, engine='openpyxl', **kwargs)

            if isinstance(data, dict): # Loaded all sheets
                self.logger.info(f"Successfully loaded all sheets from {self.source}. Sheet names: {list(data.keys())}")
                for sheet, df in data.items():
                    self.logger.debug(f"Sheet: '{sheet}', Shape: {df.shape}")
            else: # Loaded a single sheet
                self.logger.info(f"Successfully loaded sheet '{sheet_name}' from {self.source}. Shape: {data.shape}")
            return data
        except FileNotFoundError: # Should be caught by os.path.exists, but as a safeguard
            self.logger.error(f"File not found (pd.read_excel): {self.source}")
            raise
        except ValueError as ve: # Handles incorrect sheet names, or other pandas value errors
            self.logger.error(f"ValueError loading Excel file {self.source} (sheet_name: {sheet_name}): {ve}")
            raise
        except Exception as e: # Catch other potential errors (e.g., openpyxl specific, corrupted file)
            ...
            self.logger.error(f"An unexpected error occurred while loading {self.source} (sheet_name: {sheet_name}): {e}")
            raise
    ...
    def save_data(self, data, target_path=None, sheet_name='Sheet1', **kwargs):
        ...
        try:
            if isinstance(data, pd.DataFrame):
                ...
                self.logger.info(f"Successfully saved single DataFrame to {target_path} in sheet '{sheet_name}'")
            elif isinstance(data, dict):
                # Ensure all values in dict are DataFrames
                if not all(isinstance(df, pd.DataFrame) for df in data.values()):
                    self.logger.error("If data is a dict, all its values must be pandas DataFrames.")
                    raise TypeError("If data is a dict, all its values must be pandas DataFrames.")

                with pd.ExcelWriter(target_path, engine='openpyxl') as writer:
                    for sheet, df in data.items():
                        df.to_excel(writer, sheet_name=sheet, index=index, **kwargs)
                self.logger.info(f"Successfully saved dictionary of DataFrames to {target_path}. Sheets: {list(data.keys())}")
            else:
                self.logger.error(f"Unsupported data type: {type(data)}. Must be a pandas DataFrame or a dictionary of DataFrames.")
...
if __name__ == '__main__':
    ...
    try:
        ...
        main_logger.info(f"Created dummy Excel file: {sample_excel_path} with sheets: SheetAlpha, SheetBeta")
    except Exception as e:
    ...
    try:
        ...
        main_logger.info(f"Loaded first sheet (index 0) data type: {type(loaded_df_first)}")
    ...
    try:
        ...
        if isinstance(loaded_all_sheets, dict):
            ...
            pd.testing.assert_frame_equal(loaded_all_sheets['SheetAlpha'], df1_to_create)
    ...
    try:
    ...
    except ValueError as e: # openpyxl/pandas typically raises ValueError for non-existent sheet
        ...
        main_logger.error(f"Caught unexpected error for non-existent sheet: {e}", exc_info=True)

    main_logger.info("\n--- Testing load_data (Non-existent file) ---")
    handler_non_existent_file = ExcelHandler(source=os.path.join(temp_dir,'non_existent.xlsx'))
    try:
    ...
    # --- Test save_data ---
    df_single_save = pd.DataFrame({'Name': ['ProductA', 'ProductB'], 'Price': [100, 150]})
    dict_multi_save = {
    ...
    handler_save = ExcelHandler(source="dummy_source_for_saving.xlsx") # Source not used for saving with target_path

    main_logger.info("\n--- Testing save_data (Single DataFrame to new file) ---")
    try:
        handler_save.save_data(df_single_save, target_path=output_single_sheet_path, sheet_name='Products')
        ...
        if os.path.exists(output_single_sheet_path):
            ...
            reloaded_single = pd.read_excel(output_single_sheet_path, sheet_name='Products', engine='openpyxl')
    ...
    try:
        ...
        if os.path.exists(output_multi_sheet_path):
            ...
            # Verify content by reloading
            reloaded_multi = pd.read_excel(output_multi_sheet_path, sheet_name=None, engine='openpyxl')
            assert 'Sales' in reloaded_multi
            assert 'Inventory' in reloaded_multi
            pd.testing.assert_frame_equal(reloaded_multi['Sales'], dict_multi_save['Sales'])
            pd.testing.assert_frame_equal(reloaded_multi['Inventory'], dict_multi_save['Inventory'])
    ...
    try:
        handler_save.save_data([1,2,3], target_path=os.path.join(temp_dir,"unsupported.xlsx"))
    ...
    try:
        bad_dict_data = {"Sheet1": pd.DataFrame({'A':[1]}), "Sheet2": "not a dataframe"}
        handler_save.save_data(bad_dict_data, target_path=os.path.join(temp_dir,"bad_dict.xlsx"))
    except TypeError as e:
        main_logger.error(f"Caught expected TypeError for dict with non-DataFrame: {e}")
    except Exception as e:
        main_logger.error(f"Caught unexpected error (dict with non-DataFrame): {e}", exc_info=True)
    ...
    try:
        paths_to_remove = [
            sample_excel_path, output_single_sheet_path, output_multi_sheet_path,
            os.path.join(temp_dir, "unsupported.xlsx"), os.path.join(temp_dir, "bad_dict.xlsx")
        ]
