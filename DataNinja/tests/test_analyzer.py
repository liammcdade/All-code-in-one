...
class TestAnalyzerInitialization(unittest.TestCase):
    ...
    def test_init_with_empty_dataframe(self):
        ...
        self.assertIn("Warning: Initialized DataAnalyzer with an empty DataFrame.", output)
    ...
    def test_init_with_ragged_list_raises_valueerror(self):
        ...
        with self.assertRaisesRegex(ValueError, "Could not convert list to DataFrame for analysis"):
...
class TestGetSummaryStatistics(unittest.TestCase):
    ...
    def test_default_summary_statistics_include_all(self):
        ...
        assert_frame_equal(stats.sort_index(axis=1), expected_stats.sort_index(axis=1), check_dtype=False)

    def test_summary_statistics_for_specific_columns(self):
        stats = self.analyzer.get_summary_statistics(columns=['NumericCol', 'StringCol'])
    ...
    def test_summary_statistics_include_numeric(self):
        ...
        assert_frame_equal(stats.sort_index(axis=1), expected_stats.sort_index(axis=1), check_dtype=True)
    ...
    def test_summary_statistics_with_non_existent_columns(self):
        with Capturing() as output:
            stats = self.analyzer.get_summary_statistics(columns=['NumericCol', 'NonExistentCol'])
        self.assertIn("Warning: Columns not found and will be ignored: ['NonExistentCol']", output)
        expected_stats = self.data[['NumericCol']].describe(include='all')
        assert_frame_equal(stats.sort_index(axis=1), expected_stats.sort_index(axis=1), check_dtype=False)

    def test_summary_statistics_all_specified_columns_missing(self):
        with Capturing() as output:
            stats = self.analyzer.get_summary_statistics(columns=['NonExistentCol1', 'NonExistentCol2'])
        self.assertIn("Warning: None of the specified columns for summary statistics were found.", output)
        self.assertTrue(stats.empty)
...
class TestGetCorrelationMatrix(unittest.TestCase):
    def setUp(self):
        ...
        self.analyzer_mixed = DataAnalyzer(pd.DataFrame({
            'Num1': [1,2,3], 'Str1': ['a','b','c'], 'Num2': [4,5,6]
        }))
        self.analyzer_single_numeric = DataAnalyzer(pd.DataFrame({'Num': [1,2,3], 'Str': ['a','b','c']}))
        self.empty_analyzer = DataAnalyzer(pd.DataFrame())
    ...
    def test_correlation_ignores_non_numeric_if_specified_in_columns(self):
        ...
        with Capturing() as output:
             corr = self.analyzer_numeric.get_correlation_matrix(columns=['A', 'D_str', 'C'])
    ...
    def test_correlation_fewer_than_two_numeric_columns(self):
        ...
        with Capturing() as output: # Also test if only one numeric column is specified
            corr_specified = self.analyzer_numeric.get_correlation_matrix(columns=['A', 'D_str'])
    ...
    def test_correlation_with_non_existent_columns(self):
        with Capturing() as output:
            corr = self.analyzer_numeric.get_correlation_matrix(columns=['A', 'NonExistent', 'C'])
        self.assertIn("Warning: Columns for correlation not found and will be ignored: ['NonExistent']", output)
        expected_corr = self.data_numeric[['A', 'C']].corr(method='pearson')
        assert_frame_equal(corr, expected_corr)

    def test_correlation_no_numeric_columns_found(self):
        analyzer_no_numeric = DataAnalyzer(pd.DataFrame({'S1': ['a','b'], 'S2': ['c','d']}))
        with Capturing() as output:
            corr = analyzer_no_numeric.get_correlation_matrix()
        self.assertTrue(corr.empty)
        self.assertIn("Warning: No numeric columns found to calculate correlation.", output[0])
...
class TestGetValueCounts(unittest.TestCase):
    ...
    def test_value_counts_on_column_with_all_nan(self):
        ...
        expected_counts = self.data['ColC_empty'].value_counts() # Should be empty Series
        self.assertTrue(counts.empty)
        assert_series_equal(counts, expected_counts)
...
class TestAnalyzeDataOrchestration(unittest.TestCase):
    ...
    def test_analyze_data_multiple_operations(self):
        ...
        assert_frame_equal(results['get_summary_statistics'].sort_index(axis=1), expected_summary.sort_index(axis=1), check_dtype=False)
    ...
    def test_analyze_data_empty_operations_list_defaults_to_summary(self):
        with Capturing() as output: # Captures "Performing default summary statistics"
            results = self.analyzer.analyze_data(operations=[]) # Changed from None to []
        self.assertIn("No specific analyses requested. Performing default summary statistics.", output)
        self.assertIn('summary_statistics', results) # Default key is 'summary_statistics'
        expected_summary = self.data.describe(include='all') # Default of get_summary_statistics
        assert_frame_equal(results['summary_statistics'].sort_index(axis=1), expected_summary.sort_index(axis=1), check_dtype=False)

    def test_analyze_data_none_operations_list_defaults_to_summary(self):
        with Capturing() as output:
            results = self.analyzer.analyze_data(analysis_types=None) # Explicitly pass None
        self.assertIn("No specific analyses requested. Performing default summary statistics.", output)
    ...
    def test_analyze_data_unknown_method(self):
        ...
        with Capturing() as output:
            results = self.analyzer.analyze_data(analysis_plan)
        self.assertIn("Warning: Unknown analysis method 'non_existent_method'. Skipping.", output)
        self.assertIn('non_existent_method', results)
        self.assertIn('error', results['non_existent_method'])
        self.assertEqual(results['non_existent_method']['error'], "Unknown method non_existent_method")
    ...
    def test_analyze_data_method_raises_exception(self):
        ...
        analysis_plan = [{'method': 'get_value_counts', 'params': {'column': 'NonExistentColumnForError'}}]
