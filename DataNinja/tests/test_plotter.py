...
class TestPlotterInitialization(unittest.TestCase):
    ...
    def test_init_with_empty_dataframe(self):
        ...
        self.assertIn("Warning: Initialized DataPlotter with an empty DataFrame.", output)
...
class TestCreatePlotMethodDispatch(unittest.TestCase):
    ...
    def test_create_plot_unsupported_type(self):
        with Capturing() as output:
            ...
            with patch.object(self.plotter, 'plot_histogram') as mock_hist, \
                 patch.object(self.plotter, 'plot_scatter') as mock_scatter, \
                 patch.object(self.plotter, 'plot_bar') as mock_bar:
        ...
        self.assertIn("Warning: Plot type 'unknown_plot_type' is not supported", output[0])
...
class TestSpecificPlotMethodExecution(unittest.TestCase):
    ...
    def test_plot_histogram_saves_file_via_create_plot(self, mock_savefig):
        ...
        with patch('matplotlib.pyplot.show') as mock_show: # Ensure show is not called if saving
            fig = self.plotter.create_plot('histogram', column='NumericCol', save_path=save_path)
    ...
    def test_plot_histogram_direct_call_non_numeric_column(self):
        ...
        self.assertIn("Warning: Column 'CategoryCol' is not numeric. Histogram may not be meaningful.", output)
    ...
    def test_plot_scatter_runs_via_create_plot_no_error(self, mock_show):
        try:
            fig = self.plotter.create_plot('scatter', x_column='NumericCol', y_column='NumericCol2')
            self.assertIsNotNone(fig)
            mock_show.assert_called_once()
        except Exception as e:
    ...
    def test_plot_scatter_direct_call_missing_column(self):
        with Capturing() as output:
            fig = self.plotter.plot_scatter(x_column='NonExistent', y_column='NumericCol2')
        self.assertIsNone(fig)
        self.assertIn("One or both columns ('NonExistent', 'NumericCol2') not found in data.", output)

    # Test plot_bar
    @patch('matplotlib.pyplot.show')
    def test_plot_bar_runs_via_create_plot_no_error(self, mock_show):
        try:
            fig = self.plotter.create_plot('bar', x_column='CategoryCol', y_column='NumericCol')
            self.assertIsNotNone(fig)
            mock_show.assert_called_once()
        except Exception as e:
    ...
    def test_plot_bar_estimator_count_via_create_plot(self, mock_show):
        try:
            fig = self.plotter.create_plot('bar', x_column='CategoryCol', y_column='NumericCol', estimator='count')
            self.assertIsNotNone(fig)
            mock_show.assert_called_once()
        except Exception as e:
            self.fail(f"create_plot for bar with estimator='count' raised an exception: {e}")
    ...
    def test_plot_bar_direct_call_missing_y_column(self):
        ...
        self.assertIsNone(fig)
        self.assertIn("One or both columns ('CategoryCol', 'NonExistentY') not found in data.", output)
    ...
    def test_plot_bar_direct_call_non_numeric_y_for_mean_estimator(self):
        ...
        with Capturing() as output:
            fig = self.plotter.plot_bar(x_column='CategoryCol', y_column='StringY', estimator='mean')
        self.assertIsNotNone(fig) # Plotter proceeds, seaborn might error or produce weird plot
        self.assertIn("Warning: y_column 'StringY' must be numeric for estimators like 'mean' or 'sum'.", output)
        plt.close(fig)
...
class TestPlottingWithEmptyOrInvalidData(unittest.TestCase):
    def setUp(self):
        ...
        self.plotter_invalid_internal_data = DataPlotter(pd.DataFrame({'A':[1]})) # Valid init
        self.plotter_invalid_internal_data.data = "not a dataframe" # Force invalid state
    ...
    def test_create_plot_with_empty_dataframe_in_plotter(self, mock_show):
        ...
        self.assertIsNone(fig)
        self.assertIn("Data is not a valid DataFrame or is empty. Cannot create plot.", output)
        mock_show.assert_not_called()

    @patch('matplotlib.pyplot.show')
    def test_create_plot_with_invalid_internal_data_in_plotter(self, mock_show):
        with Capturing() as output:
            fig = self.plotter_invalid_internal_data.create_plot('histogram', column='A')
        self.assertIsNone(fig)
        self.assertIn("Data is not a valid DataFrame or is empty. Cannot create plot.", output)
        mock_show.assert_not_called()
