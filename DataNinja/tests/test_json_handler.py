...
class TestJSONHandlerInitialization(unittest.TestCase):
    ...
    def test_get_source_info(self):
        handler = JSONHandler(source=self.sample_json_path)
        self.assertEqual(handler.get_source_info(), f"Data source: {self.sample_json_path}")
...
class TestJSONHandlerLoadData(unittest.TestCase):
    def setUp(self):
        ...
        self.json_handler_logger = logging.getLogger("DataNinja.formats.json_handler.JSONHandler")
    ...
    def test_load_json_lines_to_dataframe(self):
        ...
        loaded_df = handler.load_data(lines=True, orient='records') # lines=True is key for .jsonl
    ...
    def test_load_nested_json_falls_back_to_json_load(self):
        ...
        self.assertTrue(any("Pandas failed to parse JSON" in msg for msg in self.log_capture_handler.get_messages()))
    ...
    def test_load_malformed_json_raises_json_decode_error(self):
        ...
        with self.assertRaises(json.JSONDecodeError):
            handler.load_data()
        self.assertTrue(any("JSONDecodeError parsing" in msg for msg in self.log_capture_handler.get_messages()))
...
class TestJSONHandlerSaveData(unittest.TestCase):
    def setUp(self):
        ...
        self.output_subdir_path = os.path.join(self.temp_dir, 'subdir_save', 'output_sub.json')
        self.handler = JSONHandler(source="dummy_source_for_saving.json") # Source not used for saving
    ...
    def test_save_dataframe_orient_table_with_indent(self):
        ...
        self.handler.save_data(df_to_save, target_path=self.output_path, orient='table', indent=4)
    ...
    def test_save_python_dictionary(self):
        ...
        self.handler.save_data(dict_to_save, target_path=self.output_path, indent=2, sort_keys=True)
        ...
        with open(self.output_path, 'r') as f:
            raw_content = f.read()
        self.assertTrue(raw_content.find('"project_name"') < raw_content.find('"status"'))


    def test_save_python_list(self):
        list_to_save = [{'item_id': 'A1'}, {'item_id': 'B2', 'props': [1,2,3]}]
        self.handler.save_data(list_to_save, target_path=self.output_path, indent=None) # No pretty print
    ...
    def test_save_data_no_target_path_raises_valueerror(self):
        ...
        with self.assertRaisesRegex(ValueError, "Target path for saving JSON data is required."):
    ...
    def test_save_data_unsupported_type_raises_typeerror(self):
        ...
        with self.assertRaisesRegex(TypeError, "Unsupported data type for saving to JSON"):
    ...
    def test_save_data_non_json_serializable_content_in_dict_raises_typeerror(self):
        ...
        with self.assertRaises(TypeError) as context: # json.dump raises TypeError for non-serializable
            self.handler.save_data(data_with_datetime_object, target_path=self.output_path)
