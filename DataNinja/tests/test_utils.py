...
class TestLoadConfig(unittest.TestCase):
    def setUp(self):
        ...
        self.malformed_config_path = os.path.join(self.temp_dir, 'malformed_config.json')
        self.empty_json_path = os.path.join(self.temp_dir, 'empty.json') # Valid empty JSON like {}
        self.empty_file_path = os.path.join(self.temp_dir, 'empty_file.json') # Zero bytes
        ...
        with open(self.malformed_config_path, 'w') as f:
            f.write("{'key': 'value',") # Malformed JSON (single quotes, trailing comma)
    ...
    def test_load_non_existent_config(self):
        ...
        self.assertTrue(any(f"Warning: Configuration file '{non_existent_path}' not found." in line for line in output))

    def test_load_malformed_config(self):
        with Capturing() as output:
            config = utils.load_config(self.malformed_config_path)
        self.assertIsNone(config)
        self.assertTrue(any(f"Warning: Error decoding JSON from '{self.malformed_config_path}'." in line for line in output))
    ...
    def test_load_empty_file_not_valid_json(self):
        ...
        self.assertIsNone(config)
        self.assertTrue(any(f"Warning: Error decoding JSON from '{self.empty_file_path}'." in line for line in output))
...
class TestSetupLogging(unittest.TestCase):
    def setUp(self):
        ...
        self.loggers_to_clear = ['TestLogger.Setup', 'TestLogger.Level',
                                 'TestLogger.File', 'TestLogger.NoFile',
                                 'DataNinjaApp', 'MyModule.Test']
    ...
    def test_set_log_level(self):
        logger_debug = utils.setup_logging(log_level=logging.DEBUG, module_name='TestLogger.Level')
        self.assertEqual(logger_debug.level, logging.DEBUG)

        # Test with string level
        logger_info = utils.setup_logging(log_level='INFO', module_name='TestLogger.Level')
    ...
    def test_log_file_creation_and_write_explicit_log_file(self):
        logger = utils.setup_logging(log_level=logging.INFO, module_name='TestLogger.File', log_file=self.log_file_path)
    ...
    def test_log_file_for_dataninjaapp_module_default_path(self):
        ...
        with patch('DataNinja.core.utils.DEFAULT_LOG_FILE', temp_default_log_path):
            logger = utils.setup_logging(module_name="DataNinjaApp", log_level=logging.INFO) # log_file is None by default

        self.assertTrue(os.path.exists(temp_default_log_path))
        self.assertTrue(any(isinstance(h, logging.FileHandler) and h.baseFilename == temp_default_log_path for h in logger.handlers))


    def test_no_log_file_if_not_specified_and_not_dataninjaapp(self):
        logger = utils.setup_logging(module_name='TestLogger.NoFile', log_file=None) # Explicitly None
        ...
        logger.addHandler(log_capture_handler) # Add to the logger returned by setup_logging
        ...
        self.assertTrue(any("Outputting to console only." in msg for msg in log_capture_handler.records))
    ...
    def test_multiple_calls_do_not_duplicate_handlers(self):
        ...
        logger_second = utils.setup_logging(module_name=module_name, log_level=logging.DEBUG)
        num_handlers_second_call = len(logger_second.handlers)

        self.assertEqual(num_handlers_second_call, num_handlers_first_call) # Should be same or based on re-config logic
...
class TestEnsureDirectoryExists(unittest.TestCase):
    ...
    def test_creation_oserror(self, mock_makedirs):
        ...
        self.assertTrue(any(f"Error creating directory {error_dir}" in line for line in output))
