...
class TestExcelHandlerInitialization(unittest.TestCase):
    ...
    def test_initialization_valid_source(self):
        ...
        self.assertIsInstance(handler.logger, logging.Logger) # Check logger is initialized

    def test_get_source_info(self):
        handler = ExcelHandler(source=self.sample_excel_path)
        self.assertEqual(handler.get_source_info(), f"Data source: {self.sample_excel_path}")
...
class TestExcelHandlerLoadData(unittest.TestCase):
    def setUp(self):
        ...
        self.malformed_excel_path = os.path.join(self.temp_dir, 'malformed.xlsx') # Text file
        ...
        self.excel_handler_logger = logging.getLogger("DataNinja.formats.excel_handler.ExcelHandler")
        self.excel_handler_logger.addHandler(self.log_capture_handler)
        self.excel_handler_logger.setLevel(logging.DEBUG) # Ensure we capture all levels
    ...
    def test_load_with_kwargs_skiprows_usecols(self):
        ...
        df_to_write.to_excel(path_kwargs_excel, sheet_name='TestSheet', index=False, header=False) # No header written by pandas
        ...
        loaded_df = handler.load_data(sheet_name='TestSheet', skiprows=2, usecols=[1,3], header=0)
    ...
    def test_load_malformed_excel_file_raises_error(self):
        ...
        self.assertTrue(any("An unexpected error occurred" in msg for msg in self.log_capture_handler.get_messages()))
...
class TestExcelHandlerSaveData(unittest.TestCase):
    def setUp(self):
        ...
        self.output_subdir_excel_path = os.path.join(self.temp_dir, 'subdir_test', 'output_in_subdir.xlsx')
        self.df_single = pd.DataFrame({'Product': ['Apples', 'Bananas'], 'Qty': [100, 150]})
        self.dict_dfs = {
            'Sales': pd.DataFrame({'Rep': ['Joe', 'Liz'], 'Total': [5000, 6500]}),
            'Costs': pd.DataFrame({'Category': ['Office', 'Travel'], 'Amount': [500, 1200]})
        }
    ...
    def test_save_single_dataframe(self):
        self.handler.save_data(self.df_single, target_path=self.output_excel_path, sheet_name='Inventory')
        self.assertTrue(os.path.exists(self.output_excel_path))
        reloaded_df = pd.read_excel(self.output_excel_path, sheet_name='Inventory', engine='openpyxl')
        assert_frame_equal(reloaded_df, self.df_single)

    def test_save_dict_of_dataframes(self):
        self.handler.save_data(self.dict_dfs, target_path=self.output_excel_path) # Overwrites if exists
        self.assertTrue(os.path.exists(self.output_excel_path))
        reloaded_data = pd.read_excel(self.output_excel_path, sheet_name=None, engine='openpyxl')
    ...
    def test_save_data_with_kwargs_index_and_startrow(self):
        ...
        self.handler.save_data(self.df_single, target_path=self.output_excel_path, sheet_name='WithKwargs',
        ...
        reloaded_df = pd.read_excel(self.output_excel_path, sheet_name='WithKwargs',
                                    engine='openpyxl', header=2, index_col=0)
        assert_frame_equal(reloaded_df, self.df_single)


    def test_save_data_creates_subdirectory(self):
        self.handler.save_data(self.df_single, target_path=self.output_subdir_excel_path, sheet_name='SubdirTest')
    ...
    def test_save_data_no_target_path_raises_valueerror(self):
        with self.assertRaisesRegex(ValueError, "Target path for saving Excel data is required."):
    ...
    def test_save_data_invalid_data_type_raises_typeerror(self):
        ...
        with self.assertRaisesRegex(TypeError, "Data must be a pandas DataFrame or a dictionary of DataFrames."):
    ...
    def test_save_data_dict_with_invalid_value_type_raises_typeerror(self):
        dict_with_invalid_value = {'Sheet1': self.df_single, 'Sheet2': "not a DataFrame"}
        with self.assertRaisesRegex(TypeError, "If data is a dict, all its values must be pandas DataFrames."):
            self.handler.save_data(dict_with_invalid_value, target_path=self.output_excel_path)
