...
class TestRemoveMissingValues(unittest.TestCase):
    ...
    def test_drop_rows_default_any_na(self):
        ...
        cleaned_df = self.cleaner_instance.remove_missing_values(self.data_with_nan.copy())
        expected_df = self.data_with_nan.dropna(axis=0) # Default for dropna
        assert_frame_equal(cleaned_df.reset_index(drop=True), expected_df.reset_index(drop=True))

    def test_drop_rows_with_subset(self):
        # Remove rows if NA is in column 'B' or 'C'
        cleaned_df = self.cleaner_instance.remove_missing_values(self.data_with_nan.copy(), subset=['B', 'C'])
    ...
    def test_drop_rows_with_threshold(self):
        ...
        cleaned_df = self.cleaner_instance.remove_missing_values(self.data_with_nan.copy(), threshold=3)
    ...
    def test_drop_cols_strategy(self):
        ...
        cleaned_df = self.cleaner_instance.remove_missing_values(df_with_nan_col, strategy='drop_cols')
    ...
    def test_unsupported_strategy(self):
        with Capturing() as output:
            cleaned_df = self.cleaner_instance.remove_missing_values(self.data_with_nan.copy(), strategy='unknown_strategy')
    ...
    def test_non_dataframe_input(self):
        ...
        self.assertIn("Data is not a DataFrame. Cannot remove missing values effectively.", output[0])
...
class TestConvertColumnType(unittest.TestCase):
    ...
    def test_convert_to_int(self):
        ...
        self.assertEqual(cleaned_df['A'].dtype, np.int64) # Or np.int32 depending on system
        assert_series_equal(cleaned_df['A'], pd.Series([1, 2, 3, 4], name='A', dtype=np.int64))

    def test_convert_to_float(self):
        cleaned_df = self.cleaner_instance.convert_column_type(self.df.copy(), 'B', float)
        self.assertEqual(cleaned_df['B'].dtype, np.float64)
        assert_series_equal(cleaned_df['B'], pd.Series([10.1, 20.2, 30.3, 40.4], name='B', dtype=np.float64))
    ...
    def test_convert_to_str(self):
        ...
        assert_series_equal(cleaned_df['D'], pd.Series(['True', 'False', 'True', 'False'], name='D'))
    ...
    def test_convert_non_convertible_value_error_caught(self):
        ...
        with Capturing() as output: # Capture print output from the method
            cleaned_df = self.cleaner_instance.convert_column_type(self.df.copy(), 'E', int)
        # The method catches the error and prints a message. The column remains unchanged.
        self.assertTrue(any("Error converting column 'E' to <class 'int'>" in line for line in output))
        assert_series_equal(cleaned_df['E'], self.df['E'])


    def test_convert_non_existent_column(self):
        with Capturing() as output: # Capture print output
            cleaned_df = self.cleaner_instance.convert_column_type(self.df.copy(), 'Z', int)
        self.assertIn("Warning: Column 'Z' not found in DataFrame. Skipping type conversion.", output[0])
        assert_frame_equal(cleaned_df, self.df) # DataFrame should be unchanged

    def test_empty_dataframe(self):
        df_empty = pd.DataFrame({'X': pd.Series([], dtype=object)}) # Empty series with object type
        cleaned_df = self.cleaner_instance.convert_column_type(df_empty.copy(), 'X', int)
        ...
        with Capturing() as output:
             cleaned_df_no_cols = self.cleaner_instance.convert_column_type(df_empty_no_cols.copy(), 'X', int)
        self.assertIn("Warning: Column 'X' not found in DataFrame. Skipping type conversion.", output[0])
...
class TestRemoveDuplicates(unittest.TestCase):
    ...
    def test_remove_duplicates_default_keep_first(self):
        cleaned_df = self.cleaner_instance.remove_duplicates(self.data_with_duplicates.copy())
        expected_df = self.data_with_duplicates.drop_duplicates(keep='first')
        assert_frame_equal(cleaned_df.reset_index(drop=True), expected_df.reset_index(drop=True))

    def test_remove_duplicates_keep_last(self):
        cleaned_df = self.cleaner_instance.remove_duplicates(self.data_with_duplicates.copy(), keep='last')
    ...
    def test_remove_duplicates_keep_false(self):
        ...
        cleaned_df = self.cleaner_instance.remove_duplicates(self.data_with_duplicates.copy(), keep=False)
    ...
    def test_remove_duplicates_with_subset(self):
        ...
        cleaned_df = self.cleaner_instance.remove_duplicates(self.data_with_duplicates.copy(), subset=['A', 'B'])
        expected_df = self.data_with_duplicates.drop_duplicates(subset=['A', 'B'], keep='first')
        assert_frame_equal(cleaned_df.reset_index(drop=True), expected_df.reset_index(drop=True))
...
class TestCleanDataOrchestration(unittest.TestCase):
    ...
    def test_clean_data_multiple_operations(self):
        ...
        operations = [
            ...
            {'method': 'convert_column_type', 'params': {'column': 'Age', 'new_type': float}},
            ...
            {'method': 'convert_column_type', 'params': {'column': 'Salary', 'new_type': float}}, # This will log error for 'N/A'
            {'method': 'remove_duplicates', 'params': {'subset': ['ID', 'Name'], 'keep': 'first'}}
        ]
        ...
        expected_final_df = df_step3.drop_duplicates(subset=['ID', 'Name'], keep='first').reset_index(drop=True)

        with Capturing() as output: # Capture prints
            cleaner.clean_data(operations=operations)
        cleaned_df = cleaner.get_cleaned_data()

        self.assertTrue(any("Error converting column 'Salary' to <class 'float'>" in line for line in output))
        assert_frame_equal(cleaned_df.reset_index(drop=True), expected_final_df.reset_index(drop=True))
    ...
    def test_clean_data_empty_operations_list(self):
        ...
        cleaned_df = cleaner.get_cleaned_data()
        self.assertIn("No specific cleaning operations provided. Applying default steps (if any).", output[0])
        assert_frame_equal(cleaned_df, original_df) # self.data is updated with the copy from start of clean_data
    ...
    def test_clean_data_unknown_method(self):
        ...
        with Capturing() as output:
            cleaner.clean_data(operations=operations)
        cleaned_df = cleaner.get_cleaned_data()
        self.assertIn("Warning: Unknown cleaning method 'non_existent_method'. Skipping.", output[0])
    ...
    def test_clean_data_on_non_dataframe_at_init(self):
        ...
        cleaner_bad_data = DataCleaner([1, "test", [2,3]]) # This init stores data as is
        ...
        self.assertTrue(any("Warning: Data is not a pandas DataFrame." in line for line in output))
        self.assertEqual(cleaned_output, original_bad_data) # Should return original non-DataFrame data
