...
class TestYAMLHandlerInitialization(unittest.TestCase):
    ...
    def test_get_source_info(self):
        handler = YAMLHandler(source=self.sample_yaml_path)
        self.assertEqual(handler.get_source_info(), f"Data source: {self.sample_yaml_path}")
...
class TestYAMLHandlerLoadData(unittest.TestCase):
    def setUp(self):
        ...
        self.empty_content_yaml_path = os.path.join(self.temp_dir, 'empty_content.yaml') # contains 'null'
        self.utf16_yaml_path = os.path.join(self.temp_dir, 'utf16.yaml')


        self.dict_data = {'key': 'value', 'number': 123, 'boolean': True, 'unicode': 'éàçüö'}
        ...
        self.nested_data = {'config': {'version': 1.2, 'params': ['a', 'b_éàç']}, 'data': [1,2,3]}
    ...
    def test_load_malformed_yaml_raises_yaml_error(self):
        ...
        with self.assertRaises(yaml.YAMLError): # PyYAML raises YAMLError or its subclasses
            handler.load_data()
...
class TestYAMLHandlerSaveData(unittest.TestCase):
    def setUp(self):
        ...
        self.output_subdir_path = os.path.join(self.temp_dir, 'subdir_save', 'output_sub.yaml')
        self.handler = YAMLHandler(source="dummy_source_for_saving.yaml") # Source not used for saving

    def tearDown(self):
        shutil.rmtree(self.temp_dir)

    def test_save_dictionary(self):
        data_to_save = {'name': 'Test Config', 'version': 1.0, 'active': True, 'unicode_val': 'éàç'}
    ...
    def test_save_nested_structure(self):
        data_to_save = {'level1': {'name': 'L1', 'children': [{'name': 'L2_child1_éàç', 'value': 100}]}}
        self.handler.save_data(data_to_save, target_path=self.output_path)
        reloaded_data = YAMLHandler(source=self.output_path).load_data()
        self.assertEqual(reloaded_data, data_to_save)
    ...
    def test_save_with_kwargs_indent_sort_keys_allow_unicode(self):
        ...
        self.handler.save_data(data_to_save, target_path=self.output_path,
                               indent=4, sort_keys=True, allow_unicode=True)
        ...
        self.assertTrue(content.find('a_key:') < content.find('c_key:') < content.find('z_key:'))
    ...
    def test_save_data_no_target_path_raises_valueerror(self):
        data_to_save = {'data': [1]}
        with self.assertRaisesRegex(ValueError, "Target path for saving YAML data is required."):
    ...
    def test_save_data_non_yaml_serializable_raises_error(self):
        ...
        self.assertTrue("represent" in str(context.exception).lower() or \
                        "serializable" in str(context.exception).lower())
