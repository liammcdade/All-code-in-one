...
class TestSQLiteHandlerSaveAndLoadData(unittest.TestCase):
    def setUp(self):
        ...
        self.df3_other_schema = pd.DataFrame({'colX': [1.1, 2.2], 'colY': [True, False]})

        # Configure a logger for the SQLiteHandler to capture its messages
        self.log_capture_handler = LogCaptureHandler()
        self.sqlite_handler_logger = logging.getLogger("DataNinja.formats.sqlite_handler.SQLiteHandler")
    ...
    def test_save_data_with_index_true(self):
        ...
        handler.save_data(df_custom_idx, table_name='table_idx_true', if_exists='replace', index=True)

        # Reload using pandas directly to check index
        conn = sqlite3.connect(self.db_path)
        reloaded_df = pd.read_sql_query("SELECT * FROM table_idx_true", conn, index_col='MyIndex')
        conn.close()
        assert_frame_equal(reloaded_df, df_custom_idx) # df_custom_idx already has index

    def test_save_data_default_index_false(self):
        handler = SQLiteHandler(source=self.db_path)
        df_custom_idx = self.df1.set_index(pd.Index(['idx1', 'idx2'], name='MyIndex'))
        # Save with default index=False
        handler.save_data(df_custom_idx, table_name='table_idx_false', if_exists='replace')
        ...
        self.assertNotIn('MyIndex', loaded_df.columns) # Check it's not a column if index=False
        self.assertNotEqual(loaded_df.index.name, 'MyIndex')
        assert_frame_equal(loaded_df, self.df1.reset_index(drop=True)) # Compare against original data without MyIndex

    def test_save_data_creates_subdirectory_for_db(self):
        handler = SQLiteHandler(source=self.db_path_subdir)
        self.assertFalse(os.path.exists(os.path.dirname(self.db_path_subdir))) # Dir shouldn't exist yet
    ...
    def test_save_data_no_table_name_raises_valueerror(self):
        handler = SQLiteHandler(source=self.db_path)
        with self.assertRaisesRegex(ValueError, "'table_name' must be provided as a non-empty string."):
    ...
    def test_save_data_not_dataframe_raises_typeerror(self):
        handler = SQLiteHandler(source=self.db_path)
        with self.assertRaisesRegex(TypeError, "Data to save must be a pandas DataFrame."):
            handler.save_data("this is not a DataFrame", table_name='bad_data_table')

    def test_save_empty_dataframe_if_exists_fail(self):
        handler = SQLiteHandler(source=self.db_path)
        empty_df = pd.DataFrame({'A': pd.Series(dtype='int'), 'B': pd.Series(dtype='object')})
        ...
        self.assertTrue("Cannot infer 'A' SQL type" in str(context.exception) or "Empty DataFrame" in str(context.exception))


    # --- Load Data Tests ---
    def test_load_data_by_table_name(self):
        handler = SQLiteHandler(source=self.db_path)
        handler.save_data(self.df3_other_schema, table_name='load_test_table', if_exists='replace')
    ...
    def test_load_data_non_existent_table_raises_operationalerror(self):
        ...
        if not os.path.exists(self.db_path):
            conn = sqlite3.connect(self.db_path); conn.close()
    ...
    def test_load_data_invalid_sql_query_raises_operationalerror(self):
        ...
        with self.assertRaises((sqlite3.OperationalError, pd.errors.DatabaseError)):
            handler.load_data(query="SELECT FROM NonExistentTable WHERE Invalid Syntax Here")

    def test_load_data_no_table_name_or_query_raises_valueerror(self):
        handler = SQLiteHandler(source=self.db_path)
        with self.assertRaisesRegex(ValueError, "Either 'table_name' or 'query' must be provided."):
            handler.load_data()

    def test_load_data_both_table_name_and_query_raises_valueerror(self):
        handler = SQLiteHandler(source=self.db_path)
        with self.assertRaisesRegex(ValueError, "Provide either 'table_name' or 'query', not both."):
    ...
    def test_load_data_from_empty_db_file_table_not_found(self):
        ...
        with self.assertRaises((sqlite3.OperationalError, sqlite3.DatabaseError, pd.errors.DatabaseError)):
