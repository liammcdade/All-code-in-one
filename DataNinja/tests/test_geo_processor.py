...
class TestCalculateDistance(unittest.TestCase):
    ...
    def test_calculate_distance_sf_la_km(self):
        distance = self.geo_proc.calculate_distance(self.lat_sf, self.lon_sf, self.lat_la, self.lon_la, unit='km')
        self.assertAlmostEqual(distance, self.dist_sf_la_km, delta=self.tolerance_km)

    def test_calculate_distance_sf_la_miles(self):
        distance = self.geo_proc.calculate_distance(self.lat_sf, self.lon_sf, self.lat_la, self.lon_la, unit='miles')
        self.assertAlmostEqual(distance, self.dist_sf_la_miles, delta=self.tolerance_miles)

    def test_calculate_distance_london_paris_km(self):
        distance = self.geo_proc.calculate_distance(self.lat_london, self.lon_london, self.lat_paris, self.lon_paris, unit='km')
        self.assertAlmostEqual(distance, self.dist_london_paris_km, delta=self.tolerance_km)

    def test_calculate_distance_london_paris_miles(self):
        distance = self.geo_proc.calculate_distance(self.lat_london, self.lon_london, self.lat_paris, self.lon_paris, unit='miles')
        self.assertAlmostEqual(distance, self.dist_london_paris_miles, delta=self.tolerance_miles)

    def test_calculate_distance_zero_distance(self):
        # Distance from a point to itself should be 0
        distance_km = self.geo_proc.calculate_distance(self.lat_sf, self.lon_sf, self.lat_sf, self.lon_sf, unit='km')
        self.assertEqual(distance_km, 0.0)
        distance_miles = self.geo_proc.calculate_distance(self.lat_sf, self.lon_sf, self.lat_sf, self.lon_sf, unit='miles')
    ...
    def test_calculate_distance_crossing_equator_and_prime_meridian(self):
        ...
        distance_km = self.geo_proc.calculate_distance(lat1, lon1, lat2, lon2, unit='km')
        self.assertAlmostEqual(distance_km, expected_km, delta=self.tolerance_km * 2) # Wider tolerance for longer distances

        distance_miles = self.geo_proc.calculate_distance(lat1, lon1, lat2, lon2, unit='miles')
        self.assertAlmostEqual(distance_miles, expected_miles, delta=self.tolerance_miles * 2)
    ...
    def test_calculate_distance_invalid_coordinate_type_raises_typeerror(self):
        with self.assertRaisesRegex(TypeError, "Coordinate lat1 must be a number"):
            self.geo_proc.calculate_distance("not_a_float", self.lon_sf, self.lat_la, self.lon_la)
        with self.assertRaisesRegex(TypeError, "Coordinate lon1 must be a number"):
            self.geo_proc.calculate_distance(self.lat_sf, "not_a_float", self.lat_la, self.lon_la)
        with self.assertRaisesRegex(TypeError, "Coordinate lat2 must be a number"):
            self.geo_proc.calculate_distance(self.lat_sf, self.lon_sf, "not_a_float", self.lon_la)
        with self.assertRaisesRegex(TypeError, "Coordinate lon2 must be a number"):
            self.geo_proc.calculate_distance(self.lat_sf, self.lon_sf, self.lat_la, "not_a_float")
    ...
    def test_calculate_distance_invalid_unit_raises_valueerror(self):
        with self.assertRaisesRegex(ValueError, "Unit must be 'km' or 'miles'."):
            self.geo_proc.calculate_distance(self.lat_sf, self.lon_sf, self.lat_la, self.lon_la, unit="furlongs")
        with self.assertRaisesRegex(ValueError, "Unit must be 'km' or 'miles'."):
            self.geo_proc.calculate_distance(self.lat_sf, self.lon_sf, self.lat_la, self.lon_la, unit="") # Empty unit
...
class TestGeocodeAddress(unittest.TestCase):
    def setUp(self):
        ...
        self.geo_processor_logger = logging.getLogger("DataNinja.plugins.geo.GeoProcessor")
        self.original_level = self.geo_processor_logger.level
        self.geo_processor_logger.addHandler(self.log_capture_handler)
        self.geo_processor_logger.setLevel(logging.INFO) # Ensure INFO messages are captured
    ...
    def test_geocode_address_returns_mocked_data(self):
        ...
        self.assertTrue(any(f"Geocoding is mocked for '{address}'. Returning fixed dummy data" in msg for msg in log_messages))
    ...
    def test_geocode_address_with_api_key_logs_key_and_returns_mocked_data(self):
        ...
        log_messages = self.log_capture_handler.get_messages()
        self.assertTrue(any(f"Received API key: {'*' * (len(api_key) - 3) + api_key[-3:]}" in msg for msg in log_messages))
        self.assertTrue(any(f"Geocoding is mocked for '{address}'. Returning fixed dummy data" in msg for msg in log_messages))
    ...
    def test_geocode_address_empty_address_raises_valueerror(self):
        with self.assertRaisesRegex(ValueError, "Address cannot be an empty or whitespace-only string."):
