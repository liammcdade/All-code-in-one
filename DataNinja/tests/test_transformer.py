...
class TestTransformerInitialization(unittest.TestCase):
    ...
    def test_init_with_empty_dataframe(self):
        ...
        self.assertIn("Warning: Initialized DataTransformer with an empty DataFrame.", output)
    ...
    def test_init_with_ragged_list_raises_valueerror(self):
        ...
        with self.assertRaisesRegex(ValueError, "Could not convert list to DataFrame for transformation"):
...
class TestScaleNumericalFeatures(unittest.TestCase):
    def setUp(self):
        self.data = pd.DataFrame({
            'Numeric1': [10., 20., 30., 40., 50.], # Use float for consistency
            'Numeric2': [1., 2., 3., 4., np.nan],
            'StringCol': ['a', 'b', 'c', 'd', 'e'],
            'Numeric3_all_same': [5., 5., 5., 5., 5.]
        })
    ...
    def test_minmax_scaling_basic(self):
        ...
        self.assertTrue((scaled_df['Numeric1'] >= 0).all() and (scaled_df['Numeric1'] <= 1).all())
        self.assertEqual(scaled_df['Numeric1'].min(), 0.0)
        self.assertEqual(scaled_df['Numeric1'].max(), 1.0)
        assert_series_equal(scaled_df['Numeric2'], self.data['Numeric2'], check_dtype=False) # Unchanged
        self.assertIn('Numeric1', self.transformer_instance.scalers) # Check scaler stored
    ...
    def test_minmax_scaling_with_nan_column_is_skipped_due_to_error(self):
        ...
        self.assertTrue(any("Error scaling column 'Numeric2'" in line for line in output))
        assert_series_equal(scaled_df['Numeric2'], self.data['Numeric2'], check_dtype=False) # Column remains unscaled
        self.assertNotIn('Numeric2', self.transformer_instance.scalers) # Scaler not stored due to error
    ...
    def test_scaling_non_numeric_column_is_skipped(self):
        ...
        self.assertIn("Warning: Column 'StringCol' is not numeric. Skipping scaling.", output)
    ...
    def test_scaling_non_existent_column_is_skipped(self):
        ...
        self.assertIn("Warning: Column 'NonExistentCol' not found for scaling. Skipping.", output)
    ...
    def test_unrecognized_scaler_type_is_skipped(self):
        ...
        with Capturing() as output:
            scaled_df = self.transformer_instance.scale_numerical_features(
                df=df_copy,
                columns=['Numeric1'],
                scaler_type='unknown_scaler'
            )
        self.assertIn("Warning: Scaler type 'unknown_scaler' not recognized. Skipping column 'Numeric1'.", output)
...
class TestEncodeCategoricalFeatures(unittest.TestCase):
    def setUp(self):
        self.data = pd.DataFrame({
            ...
            'IntCategory': [10, 20, 10, 30, 20] # Numeric but could be treated as categorical
        })
        self.transformer_instance = DataTransformer(self.data.copy())
    ...
    def test_onehot_encoding_basic(self):
        ...
        self.assertTrue((encoded_df[['Category1_A', 'Category1_B', 'Category1_C']].sum(axis=1) == 1).all())
    ...
    def test_onehot_encoding_with_drop_first(self):
        ...
        generated_cols = [col for col in encoded_df.columns if col.startswith('Category1_')]
    ...
    def test_encoding_non_existent_column_is_skipped(self):
        ...
        with Capturing() as output:
            encoded_df = self.transformer_instance.encode_categorical_features(
                df=df_copy,
                columns=['NonExistentCat'],
                encoder_type='onehot'
            )
        self.assertIn("Warning: Column 'NonExistentCat' not found for encoding. Skipping.", output)
    ...
    def test_unrecognized_encoder_type_is_skipped(self):
        ...
        self.assertIn("Warning: Encoder type 'unknown_encoder' not recognized. Skipping column 'Category1'.", output)
        assert_frame_equal(encoded_df, self.data) # Should be unchanged
...
class TestTransformDataOrchestration(unittest.TestCase):
    def setUp(self):
        self.data = pd.DataFrame({
            'Num': [10., 20., 30., np.nan], # Float for scaling, with NaN
            'Cat': ['P', 'Q', 'P', 'Q'],
    ...
    def test_transform_data_sequence_scale_and_encode(self):
        transformations = [
            {'method': 'scale_numerical_features', 'params': {'columns': ['Num'], 'scaler_type': 'minmax'}},
            {'method': 'encode_categorical_features', 'params': {'columns': ['Cat'], 'encoder_type': 'onehot', 'drop': 'first'}}
        ]
        ...
        assert_series_equal(transformed_df['Num'], self.data['Num'], check_dtype=False) # Unchanged

        # Check encoding of 'Cat'
        self.assertNotIn('Cat', transformed_df.columns)
        # Depending on which category ('P' or 'Q') is dropped first
        cat_cols_present = [col for col in transformed_df.columns if col.startswith('Cat_')]
    ...
    def test_transform_data_unknown_method_is_skipped(self):
        ...
        with Capturing() as output:
            self.transformer.transform_data(transformations)
        transformed_df = self.transformer.get_transformed_data()
        self.assertIn("Warning: Unknown transformation method 'non_existent_transform'. Skipping.", output)
    ...
    def test_transform_data_operation_fails_midway_continues(self):
        ...
        transformations = [
            {'method': 'scale_numerical_features', 'params': {'columns': ['Num'], 'scaler_type': 'minmax'}},
            {'method': 'encode_categorical_features', 'params': {'columns': ['Cat2'], 'encoder_type': 'onehot'}}
        ]
        with Capturing() as output:
            self.transformer.transform_data(transformations)
        ...
        self.assertTrue(any("Error scaling column 'Num'" in line for line in output)) # Scaling error
        assert_series_equal(transformed_df['Num'], self.data['Num'], check_dtype=False) # Num unchanged
